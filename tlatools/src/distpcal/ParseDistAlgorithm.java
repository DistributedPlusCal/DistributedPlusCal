...
*    depending on the value of DistPcalParams.AddLabels.                       *
*                                                                          *
...
import distpcal.exception.TokenizerException;
import distpcal.exception.UnrecoverableException;
import tla2tex.Debug;

public class ParseDistAlgorithm {
	private static DistPcalCharReader charReader;
	/**********************************************************************
	 * The charReader argument to the Parse method is put here so other * methods
	 * can access it. This makes the class totally thread unsafe. *
	 **********************************************************************/

	public static String currentProcedure;
	/**********************************************************************
	 * This is set by GetProcedure to the procedure's name before the * procedure
	 * body is parsed, and is reset to null afterwards. Hence, * its value is null
	 * unless in the middle of parsing a procedure. * (Its value is used to check
	 * for a <Return> not in a procedure * body.) *
	 **********************************************************************/

	/*
	 * plusLabels and minusLabels are vectors of labels that appear within the
	 * current process or procedure with + or - modifiers, respectively. Added in
	 * Version 1.5.
	 */
	...
	/**
	 * proceduresCalled is the vector of distinct names of procedures called within
	 * the current process or procedure.
	 */

	/*
	 * The following are added to record whether or not the translation needs (a)
	 * the variable pc and (b) the stuttering-when-done disjunct of Next. The
	 * gotoDoneUsed flag is set true if the algorithm contains a `goto Done'
	 * statement, in which case omitPC and omitStutteringWhenDone should be set
	 * false. The gotoUsed flag is set true if any goto is used, in which case
	 * omitPC should be set false. The gotoUsed and gotoDoneUsed flags are set by
	 * the getGoto method; the omitPC and omitStutteringWhenDone flags are set by
	 * getAlgorithm, though most of the work is done by the checkBody method.
	 * 
	 * In a more elegant implementation, this information would be saved as part of
	 * the AST object for the complete algorithm. However, since omitting these
	 * things is an addition made for Version 1.5, it's easier to modify the "code"
	 * generation by having the information available in global variables.
	 */
	...
	/**********************************************************************
	 * Set true if any variable has a default initialization. * (Added by LL on 22
	 * Aug 2007.) *
	 **********************************************************************/

	/***************************************************************************
	 * CONSTRUCTING THE AST * * For most nonterminals <NonTerm> in the simple
	 * grammar, there is a * method GetNonTerm() that processes the input from the
	 * current position * and returns an AST.NonTerm object. There may also be a
	 * method * GetNonTermSeq() that parses a sequence of <NonTerm> nodes and
	 * returns a * vector of AST.NonTerm objects. *
	 ***************************************************************************/

	public static boolean hasLabel;
	/**********************************************************************
	 * This variable is set to true by GetStmtSeq whenever a label is * found. It is
	 * used in deciding whether or not to add labels to a * uniprocess algorithm. *
	 **********************************************************************/

	public static Hashtable allLabels;
	/**********************************************************************
	 * Set by GetStmtSeq to a table of String objects containing all the *
	 * algorithm's (user-typed) labels. The strings are the keys, the * value of all
	 * entries is 0. The value of * allLabels.containsKey("foo") is true iff "foo"
	 * is a label of the * algorithm. *
	 **********************************************************************/

	public static int nextLabelNum;
	/**********************************************************************
	 * The number to be appended to DistPcalParams.LabelRoot to form the next *
	 * label to be added. *
	 **********************************************************************/

	/************************************************************************
	 * As described above, the method AddLabelsToStmtSeq determines where * labels
	 * are to be added. Added labels are reported as a bug if the * user doesn't
	 * want them, and are printed in the terminal output if * she does and has
	 * specified the -reportLabels option. The i-th entry * of the vectors
	 * addedLabels and addedLabelsLocs records the name of * the i-th added label
	 * and the location of the statement to which it * was added. *
	 ************************************************************************/
	public static Vector addedLabels;
	public static Vector addedLabelsLocs;

	/**********************************************************************
	 * Is set to the sequence of procedures. It's easier making this a * global
	 * variable than passing it as a parameter. *
	 **********************************************************************/
	public static Vector procedures;

	/**********************************************************************
	 * One of these booleans will be set true when we discover which * syntax is
	 * being used.
	 **********************************************************************/
	...
	/**********************************************************************
	 * This performs the initialization needed by the various Get... * methods,
	 * including setting charReader. It is called only by * GetAlgorithm. It's been
	 * made into a separate method for debugging, * so the various Get methods can
	 * be tested without calling * GetAlgorithm. *
	 **********************************************************************/
	public static void Init(DistPcalCharReader charR) {
		 ...
		 ******************************************************************/
		DistPcalBuiltInSymbols.Initialize();
		/*******************************************************************
		 * Initializes the hash tables of class PcalBuiltInSymbols, which * is required
		 * before methods in the Tokenize class can be called. *
		 *******************************************************************/
		AST.ASTInit();
		/*******************************************************************
		 * Performs the initialization method of the AST to create default * objects for
		 * each subclass. *
		 *******************************************************************/
	...
	public static AST getAlgorithm(DistPcalCharReader charR, boolean fairAlgorithm) throws ParseDistAlgorithmException {
		/**********************************************************************
		 * Assumes that the char reader charR is just past the string *
		 * DistPcalParams.BeginAlg that marks the character right after * "--algorithm"
		 * or "--fair". The argument fairAlgorithm is true iff * the algorithm was begun
		 * by "--fair". * * The AST that it returns does not have the col and line
		 * fields set. * If those fields might ever be used in an error message, they *
		 * should be set to the position of the DistPcalParams.BeginAlg string by *
		...
		try {
			...
			if (fairAlgorithm) {
				...
				if (!nextToken.equals(DistPcalParams.BeginFairAlg2)) {
					ParsingError("`" + DistPcalParams.BeginFairAlg + "' not followed by `"
							+ DistPcalParams.BeginFairAlg2 + "'");
				}
				DistPcalParams.FairAlgorithm = true;
			}
			...
			// we are expecting one of the follwoing
			// 1- node
			// 2- fair node
			// 2- fair +node
			if (PeekAtAlgToken(1).equals("node")
				...
				DistPCalLocation multiprocBegin = new DistPCalLocation(map.algLine, map.algColumn);
				...
				while (PeekAtAlgToken(1).equals("fair") || PeekAtAlgToken(1).equals("node")) {
					int fairness = AST.UNFAIR_PROC;
					if (PeekAtAlgToken(1).equals("fair")) {
						MustGobbleThis("fair");
						if (PeekAtAlgToken(1).equals("+")) {
							MustGobbleThis("+");
							fairness = AST.SF_NODE;
						} else {
							fairness = AST.WF_NODE;
						}
					} else {
						if (DistPcalParams.FairnessOption.equals("wf")) {
							fairness = AST.WF_NODE;
						} else if (DistPcalParams.FairnessOption.equals("sf")) {
							fairness = AST.SF_NODE;
						}
					}
				...
				while (i < multiNodes.nodes.size()) {
					...
					DistPcalDebug.reportInfo("omitStutteringWhenDoneValue: " + omitStutteringWhenDoneValue);
...
//					ExpandSendAndReceiveInStmtSeq(prcd.body, prcd.decls, vdecls);
				...
				if (addedLabels.size() > 0) {
					if (!DistPcalParams.LabelFlag) {
						AddedMessagesError();
					}

					if (DistPcalParams.ReportLabelsFlag) {
						ReportLabels();
					} else {
			...
			// ParseDistAlgorithmException means the Toolbox silently fails to translate an
			// algorithm (see Github issue at https://github.com/tlaplus/tlaplus/issues/313)
			DistPcalDebug.reportInfo("error : " + e);
	...
	/**
	 * The argument body is either the body of a uniprocess algorithm or of a
	 * process in a multiprocess algorithm. This procedure sets omitPC or
	 * omitStutteringWhenDone to false if this body implies that the pc or
	 * stuttering-when-done disjunct cannot be omitted. This is the case unless the
	 * body consists of a single `while (TRUE)' statement. In that case, the pc
	 * cannot be omitted iff there is a label in the body, which is the case iff
	 * either the AST.While object has a non-empty labDo field, or the unlabDo field
	 * contains a LabelIf or LabelEither. (The pc also cannot be omitted if the body
	 * has a procedure call, but that is checked elsewhere.)
	 * 
	 * This method incorrectly sets omitStutteringWhenDone for a multiprocess
	 * algorithm. The omitStutteringWhenDone flag should be set to true iff it is
	 * impossible for the algorithm to terminate. This is true iff there is some
	 * process that cannot terminate. However, since this method is called for each
	 * process (or process set), it causes omitStutteringWhenDone to be set false if
	 * any process can terminate, rather than being set false iff all processes can
	 * terminate. Since this is the right procedure for setting omitPC (since the pc
	 * cannot be omitted if it is needed to represent any process), I decided not to
	 * modify this method, but to work around the problem in the method that calls
	 * it. Comment added by LL on 30 Mar 2012.
	 * 
	 * @param body
	 */
	private static void checkBody(Vector body) {
		...
		if ((lblStmt.stmts == null) || (lblStmt.stmts.size() == 0)) {
			...
			DistPcalDebug.reportInfo("lblStmt.stmts == null " + (lblStmt.stmts == null));
		...
		for (int i = 0; i < whileStmt.unlabDo.size(); i++) {
			...
			if (obj.getClass().equals(AST.LabelIfObj.getClass()) || obj.getClass().equals(AST.LabelEitherObj.getClass())
	...
	public static void AddedMessagesError() throws ParseDistAlgorithmException {
		String msg = null;
		...
		;
		throw new ParseDistAlgorithmException(msg);
	}
	...
	{
		if (addedLabels.size() > 1) {
			DistPcalDebug.reportInfo("The following labels were added:");
		} else {
			DistPcalDebug.reportInfo("The following label was added:");
		}
		;
		int i = 0;
		while (i < addedLabels.size()) {
			DistPcalDebug.reportInfo(
					"  " + ((String) addedLabels.elementAt(i)) + " at " + ((String) addedLabelsLocs.elementAt(i)));
			i = i + 1;
		}
		;
		return;
	...
	public static AST.Procedure GetProcedure() throws ParseDistAlgorithmException {
...
//       CheckLabeledStmtSeq(result.body) ;
	...
	public static Vector GetPVarDecls() throws ParseDistAlgorithmException
	/**********************************************************************
	 ...
	 **********************************************************************/
	{
		...
		/********************************************************************
		 * The <PVarDecls> nonterminal is terminated by a "begin" * (p-syntax) or "{"
		 * (c-syntax) for the procedure. *
		 ********************************************************************/
	...
	public static AST.PVarDecl GetPVarDecl() throws ParseDistAlgorithmException {
	...
	public static Vector GetVarDecls() throws ParseDistAlgorithmException
	/**********************************************************************
	 ...
	 **********************************************************************/
	{
		...
		/********************************************************************
		 * The <VarDecls> nonterminal appears in two places: * * - In a <Procedure> or
		 * <Process>, where it is terminated by * a "begin" (p-syntax) or "{"
		 * (c-syntax). * * - At the beginning of a <Algorithm>, where it is terminated
		 * by * "define", "macro", "procedure", "begin" or "{", or "process" . *
		 ********************************************************************/
		while (!(PeekAtAlgToken(1).equals("begin") || PeekAtAlgToken(1).equals("{")
	...
	public static AST.VarDecl GetVarDecl() throws ParseDistAlgorithmException {
	...
	public static Vector GetChannelDecls() throws ParseDistAlgorithmException {
	...
	public static TLAExpr GetExpr() throws ParseDistAlgorithmException {
		if (LATsize != 0) {
			DistPcalDebug.ReportBug("ParseAlgorithm: GetExpr called after lookahead");
		}
		;
		TLAExpr result;
		try {
		...
		/**
		 * If the expression has any tokens, then set the origin to the region
		 * comprising the tokens. Otherwise, set the region to null.
		 */
		if (result.tokens != null && result.tokens.size() != 0) {
			DistPCalLocation begin = ((TLAToken) ((Vector) result.tokens.elementAt(0)).elementAt(0)).source.getBegin();
			...
			DistPCalLocation end = ((TLAToken) lastLineOfTokens.elementAt(lastLineOfTokens.size() - 1)).source.getEnd();
	...
	/* OBSOLETE */ public static Vector ObsoleteGetLabeledStmtSeq() throws ParseDistAlgorithmException
	/**********************************************************************
	 * Returns a (possibly null) sequence of LabeledStmt elements. This * is the
	 * obvious iterative call of GetLabeledStmt that stops when * it's not at a
	 * label. *
	 **********************************************************************/
	{
		Vector result = new Vector();
	...
	/* OBSOLETE */ public static AST.LabeledStmt ObsoleteGetLabeledStmt() throws ParseDistAlgorithmException {
		if (!IsLabelNext()) {
	...
	public static AST.While GetWhile() throws ParseDistAlgorithmException {
		...
		/******************************************************************
		 * For historical reasons, some methods expect a labDo field to * contain a
		 * vector, even though they are called before the real * labDo field is created.
		 * *
		 ******************************************************************/
		result.setOrigin(
				new Region(beginLoc, ((AST) result.unlabDo.elementAt(result.unlabDo.size() - 1)).getOrigin().getEnd()));
		return result;
	}

	public static boolean inGetMacro = false;
	/***********************************************************************
	 * This boolean equals true while inside a call to GetMacro. It is used to flag
	 * an error if a label appears within a macro.
	 * 
	 * @throws ParseDistAlgorithmException *
	 ***********************************************************************/

	/**
	 * This variable is used by GetLabel to return the location of the label for use
	 * in computing the origin field of the AST object in which the label appears.
	 * If GetLabel returns a string other than "", then getLabelLocation is the
	 * location at the beginning of the label.
	 */
	public static DistPCalLocation getLabelLocation;

	public static String GetLabel() throws ParseDistAlgorithmException
	/**********************************************************************
	 * Checks if a label comes next. If so, it gobbles it and returns * the label.
	 ...
	 **********************************************************************/
	{
		...
		if (IsLabelNext()) {
			...
			getLabelLocation = new DistPCalLocation(lastTokLine - 1, lastTokCol - 1);
			if (inGetMacro) {
	...
	public static Vector GetStmtSeq() throws ParseDistAlgorithmException {
		/**********************************************************************
		 * Parses a sequence of <Stmt>s and returns the result as a Vector of * <Stmt>
		 * nodes. It detects the end of the sequence by the * appearance of one of the
		 * following tokens * * end else elsif or * * in the p-syntax or "}" in the
		 * c-syntax. *
		 **********************************************************************/
		Vector result = new Vector();
		String tok = PeekAtAlgToken(1);
		while (!((pSyntax && (tok.equals("end") || tok.equals("else") || tok.equals("elsif") || tok.equals("or")))
				|| (cSyntax && tok.equals("}")))) {
			String nextLabel = GetLabel();
			DistPCalLocation labelLoc = getLabelLocation;
			if (cSyntax && PeekAtAlgToken(1).equals("{")) {
				/************************************************************
				 * We're using c-syntax and the next statement is a * StmtSeq. *
				 ************************************************************/
		...
		if (cSyntax && (result.size() == 0)) {
			/*****************************************************************
			 * I'm not sure if GetStmtSeq should be able to return an empty * sequence in
			 * p-syntax, but it definitely shouldn't in c-syntax * (I think). *
			 *****************************************************************/
	...
	public static AST GetStmt() throws ParseDistAlgorithmException {
	...
	public static Vector GetCStmtSeq(String lbl) throws ParseDistAlgorithmException
	/**********************************************************************
	 * Gets a c-syntax StmtSeq (enclosed in curly braces) that has a * label lbl. *
	 **********************************************************************/
	{
		/**
		 * The argument lbl must have been obtained by a call to GetLabel, and there
		 * must not have been any call to GetLabel after that call. We can therefore get
		 * the location of lbl from getLabelLocation.
		 */
		DistPCalLocation lblLocation = getLabelLocation;
		MustGobbleThis("{");
		...
		if (!lbl.equals("")) {
			...
			if (!((AST) sseq.elementAt(0)).lbl.equals("")) {
				throw new ParseDistAlgorithmException("Duplicate labeling of statement", (AST) sseq.elementAt(0));
			}
			;
	...
	public static Vector GetCStmt() throws ParseDistAlgorithmException
	/**********************************************************************
	 * Get one (possibly labeled) statement in the c-syntax, which could * be a
	 * StmtSeq, so this returns a vector of AST nodes. *
	 **********************************************************************/
	{
		String label = GetLabel();
		DistPCalLocation labelLocation = getLabelLocation;
		if (PeekAtAlgToken(1).equals("{")) {
	...
	public static boolean IsLabelNext() throws ParseDistAlgorithmException
	/**********************************************************************
	 * Peeks at the next token and perhaps at the next two characters to * determine
	 * if the next token is a label. The tricky part is * distinguishing "label : "
	 * from "var :=" when the next token isn't * a keyword. Since this is called at
	 * the beginning of a syntactic * unit that might be a labeled statement, I
	 * believe that the only * keywords it needs to check are * * while if else
	 * elsif either or end with when skip * call return assert print *
	 **********************************************************************/
	{
		String tok = PeekAtAlgToken(1);
		if ((tok.equals("while")) || (tok.equals("if")) || (tok.equals("assert")) || (tok.equals("print"))
				|| (tok.equals("else")) || (tok.equals("elsif")) || (tok.equals("either")) || (tok.equals("or"))
				|| (tok.equals("end")) || (tok.equals("with")) || (tok.equals("when")) || (tok.equals("await"))
				|| (tok.equals("skip")) || (tok.equals("call")) || (tok.equals("return")) || (tok.equals("goto"))) {
			return false;
		}
		;
		tok = charReader.peek();
		if ((tok.length() == 0) // I don't think this can happen.
				|| (tok.charAt(0) != ':') || ((tok.length() > 1) && (tok.charAt(1) == '='))) {
			return false;
		}
		;
		return true;
	...
	public static AST.LabelIf GetIf(int depth) throws ParseDistAlgorithmException
	/**********************************************************************
	 * If depth = 0, then the next token is "if" and this is the start of * an if
	 * statement. * * If depth > 0, then the next token is "elsif". An "elsif" is *
	 * equivalent to "else if" except that the "end if" of the outer "if" * also
	 * ends the inner "if". Hence, an "elsif" is parsed like an * "if", but it
	 * doesn't gobble the "end if". So there's really no * reason to count the
	 * depth, beyond knowing if it's 0 or not, but * having the depth might be
	 * useful for debugging. This parses any * "if" statement as a LabelIf. It may
	 * later be replaced by an If. * * Note: The origin is set so that the region
	 * ends at the end of the * last statement in the then or else (as appropriate).
	 * However, the * region should end at the end of the "end if" for P syntax or
	 * at the * end of the "}" if the then or else clause is a statement sequence. *
	 * To correct this, need to have GetCStmt (and perhaps GetStmtSeq) * return (by
	 * a variable) whether or not there is an ending "}" and * its location if there
	 * is one. *
	 **********************************************************************/
	{
		if (depth == 0) {
		...
		/**
		 * Set lastStmt to the AST node of the last statement in the if, which is either
		 * at the end of the end clause or, if there is none, at the end of the then
		 * clause.
		 */
		AST lastStmt = null;
		...
		/**
		 * Set the LabelIf's origin to the region from the beginning of the "if" to the
		 * end of the last then or else statement.
		 */
		result.setOrigin(new Region(beginLoc, lastStmt.getOrigin().getEnd()));
		return result;
	}

	public static AST.LabelEither GetEither() throws ParseDistAlgorithmException {
		...
		while (!done) {
			...
			if (nextClause.unlabOr.size() == 0) {
				throw new ParseDistAlgorithmException("`either' statement with empty `or' clause", result);
			}
			;
			nextClause.setOrigin(new Region(((AST) nextClause.unlabOr.elementAt(0)).getOrigin().getBegin(),
					((AST) nextClause.unlabOr.elementAt(nextClause.unlabOr.size() - 1)).getOrigin().getEnd()));
		...
		;
		if (!hasOr) {
			throw new ParseDistAlgorithmException("`either' statement has no `or'", result);
		}
		;
		result.setOrigin(
				new Region(beginLoc, ((AST) result.clauses.elementAt(result.clauses.size() - 1)).getOrigin().getEnd()));
		return result;
	}

	/**
	 * For constructing the TLA+ to PlusCal mapping, the original GetWith procedure
	 * was given a second argument and was renamed InnerGetWidth. See the comments
	 * for that method
	 * 
	 * @param depth
	 * @return
	 * @throws ParseDistAlgorithmException
	 */
	public static AST GetWith(int depth) throws ParseDistAlgorithmException {
		return InnerGetWith(depth, null);
	}

	public static AST InnerGetWith(int depth, DistPCalLocation beginLoc) throws ParseDistAlgorithmException
	/**********************************************************************
	 * A with statement has p-syntax * * <With> ::= "with" <VarEqOrIn>^+ "do"
	 * <SimpleStmt>^+ "end either" * where <VarEqOrIn> ::= <Variable> [=|\in] <Expr>
	 * ";" * * where the last ";" before the "do" is optional. The c-syntax is *
	 * analogous. * * This is converted to a nest of k With objects, where k is the
	 * * number of <VarEqOrIn> terms. If depth = 0, then the next token is * the
	 * "with". If depth > 0, then what follows is a <VarEqOrIn> * term. * * We give
	 * all the origin of all the inner with objects the same * beginning location as
	 * the outermost one, which is the beginning of * the "with" token, If depth >
	 * 0, then beginLoc is the that location. * Its value is ignored if depth = 0.
	 * This results in all the inner * With objects having the same origin as the
	 * outer one. *
	 **********************************************************************/
	{
		DistPCalLocation begLoc = beginLoc;
		if (depth == 0) {
			GobbleThis("with");
...
//		result.isEq = GobbleEqualOrIf();
		...
		if (pSyntax || !PeekAtAlgToken(1).equals(")")) {
			GobbleCommaOrSemicolon();
			/**************************************************************
			 * Gobble the ";" or "," ending the <VarEqOrIn>, which may be * eliminated
			 * before a ")" or "do". *
			 **************************************************************/
		}
		;
		...
		try {
			result.setOrigin(
					new Region(begLoc, ((AST) result.Do.elementAt(result.Do.size() - 1)).getOrigin().getEnd()));
		} catch (ArrayIndexOutOfBoundsException e) {
			throw new ParseDistAlgorithmException("Missing body of with statement", result);
		}
		
		return result;
	}

	public static AST.Assign GetAssign() throws ParseDistAlgorithmException {
	...
	public static AST.SingleAssign GetSingleAssign() throws ParseDistAlgorithmException {
	...
	public static AST.Lhs GetLhs() throws ParseDistAlgorithmException {
		...
		try {
			...
			/**
			 * beginning of LHS's region is the beginning of the variable. Its end is the
			 * end of the subscript expression, if there is one, else the end of the
			 * variable token.
			 */
	...
	public static AST.PrintS GetPrintS() throws ParseDistAlgorithmException {
	...
	public static AST.Assert GetAssert() throws ParseDistAlgorithmException {
		...
		GobbleThis(";");
		result.setOrigin(
				new Region(new DistPCalLocation(result.line - 1, result.col - 1), result.exp.getOrigin().getEnd()));
		return result;
	}

	public static AST.Skip GetSkip() throws ParseDistAlgorithmException {
	...
	public static AST.When GetWhen(boolean isWhen) throws ParseDistAlgorithmException
	/**********************************************************************
	 * Called with isWhen = true for a "when" * = false for an "await" *
	 **********************************************************************/
	{
	...
	public static AST.Call GetCall() throws ParseDistAlgorithmException {
		MustGobbleThis("call");
		...
		;
		GobbleThis(")");
		result.setOrigin(new Region(new DistPCalLocation(result.line - 1, result.col - 1),
				new DistPCalLocation(lastTokLine - 1, lastTokCol)));
		// token ")" has width 1.
		GobbleThis(";");
		/*
		 * Add the called procedure's name to proceduresCalled if it is not already in
		 * it.
		 */
		int i = 0;
		while ((i < proceduresCalled.size()) && !result.to.equals(proceduresCalled.elementAt(i))) {
			i++;
		}
		;
	...
	public static AST.Return GetReturn() throws ParseDistAlgorithmException
	/**********************************************************************
	 * Note: GetReturn should not complain if the return is not inside a * procedure
	 * because it could be in a macro that is called only from * inside a procedure.
	 * *
	 **********************************************************************/
	{
	...
	public static AST GetCallOrCallReturnOrCallGoto() throws ParseDistAlgorithmException
	/**********************************************************************
	 * Note: should not complain if it finds a return that is not inside * a
	 * procedure because it could be in a macro that is called only * from inside a
	 * procedure. *
	 **********************************************************************/
	{
		...
		if (PeekAtAlgToken(1).equals("return")) {
			MustGobbleThis("return");
			DistPCalLocation end = new DistPCalLocation(lastTokLine - 1, lastTokCol + 5);
			GobbleThis(";");
			...
			result.args = theCall.args;
			DistPCalLocation end = new DistPCalLocation(lastTokLine - 1, lastTokCol - 1 + result.to.length());
			result.setOrigin(new Region(theCall.getOrigin().getBegin(), end));
			gotoUsed = true;
	...
	public static AST.Goto GetGoto() throws ParseDistAlgorithmException {
		MustGobbleThis("goto");
		...
		result.to = GetAlgToken();
		result.setOrigin(new Region(new DistPCalLocation(result.line - 1, result.col - 1),
				new DistPCalLocation(lastTokLine - 1, lastTokCol - 1 + result.to.length())));
		gotoUsed = true;
...
//   public static AST.Macro ObsoleteGetMacro() throws ParseDistAlgorithmException 
//     /**********************************************************************
...
//     }
	...
	/**
	 * GetMacro changed by LL on 14 July 2011 to take the vector of macros as an
	 * argument that it passes to ExpandMacrosInStmtSeq. (Previously, it had passed
	 * an empty vector.) This allows macros to be called inside previously declared
	 * macros. I don't remember why this wasn't done originally, instead of not
	 * allowing macros to be called inside macros. I'm afraid that there was a
	 * reason that will appear later. For that reason, the original GetMacro
	 * procedure is contained in the comments above. To back out of this change,
	 * just remove the argument from the one call of GetMacro, and change the error
	 * message generated in ExpandMacroCall.
	 */
	public static AST.Macro GetMacro(Vector macros) throws ParseDistAlgorithmException
	/**********************************************************************
	 * This method was largely copied from GetProcedure. *
	 **********************************************************************/
	{
	...
	public static AST.MacroCall GetMacroCall() throws ParseDistAlgorithmException {
	...
	public static void AddLabelsToStmtSeq(Vector stmtseq) throws ParseDistAlgorithmException {
		InnerAddLabels(stmtseq, true, // firstLabeled
				false, // inWith
	 ...
	 * AddLabelsToStmtSeq. It returns the value false if the new value of * stmtseq
	 * has no call or return statements and no labels; otherwise it * returns true.
	 * The return value generally indicates if the calling * procedure needs to add
	 * a label to some following statement. * *
	 ***************************************************************************/
	public static boolean InnerAddLabels(Vector stmtseq,
			/********************************************************************
			 ...
			 ********************************************************************/
			Vector inAssigned, /********************************************************************
								 * The set of variables to which assignments have been made in the * current
								 * step before executing stmtseq. When calling * InnerAddLabels for an outermost
								 * `with', this will be set to the * empty set. *
								 ********************************************************************/
			Vector outAssigned) throws ParseDistAlgorithmException
	/********************************************************************
	 * This is a call-by-reference argument used to return the set of * variables to
	 * which values have been assigned in the current step * upon completing this
	 * sequence of statements. It need not be a * superset of vlbsAssigned if there
	 * could be a label inside * stmtseq that starts a new step--which is the case
	 * if inWith = * false. *
	 ********************************************************************/
	{
		int i = 0;
		...
		/******************************************************************
		 * True iff the next step requires a label--e.g., because the * previous step is
		 * a return or an if containing a label. It is * also the value returned by the
		 * entire procedure. *
		 ******************************************************************/

		boolean hadOrAddedLabel = false;
		/******************************************************************
		 * True iff a label has been found or added somewhere inside * stmtseq. *
		 ******************************************************************/
		...
		while (i < stmtseq.size()) {
			...
			if (!stmt.lbl.equals("")) {
				...
				if (inWith) {
					throw new ParseDistAlgorithmException("Label in `with' statement", stmt);
				}
				;
			}
			;
			if (nextStepNeedsLabel) {
				if (inWith) {
					throw new ParseDistAlgorithmException(
							"Statement follows `call' or `return' inside a " + "`with' statement.", stmt);
				}
				;
			...
			;
			if (stmt.getClass().equals(
					AST.LabelIfObj.getClass())) { /************************************************************
													 * This is an if statement. * - Call InnerAddLabels on the the
													 * unlabThen and * unlabElse fields. * - Set outAssigned to the
													 * union of the values * returned by the two calls. * - Set
													 * nextStepNeedsLabel to true iff either call * returns true. *
													 ************************************************************/
...
// DistPcalDebug.printVector(outAssigned, "pre outAssigned") ;
// DistPcalDebug.printVector(inAssigned, "pre inAssigned") ;
				nextStepNeedsLabel = InnerAddLabels(obj.unlabThen, false, // firstLabeled
						inWith, // inWith
						outAssigned, // inAssigned = {}
						thenAssigned); // outAssigned
				nextStepNeedsLabel = InnerAddLabels(obj.unlabElse, false, // firstLabeled
						inWith, // inWith
						outAssigned, // inAssigned = {}
						elseAssigned) // outAssigned
						|| nextStepNeedsLabel;
// DistPcalDebug.printVector(outAssigned, "mid outAssigned") ;
				Copy(Union(thenAssigned, elseAssigned), outAssigned);
// DistPcalDebug.printVector(outAssigned, "pst outAssigned") ;
			} else if (stmt.getClass().equals(
					AST.LabelEitherObj.getClass())) { /************************************************************
														 * This is a LabelEither object. The sequence of its * clauses'
														 * unlabOr fields are handled in much the same way * as
														 * unlabThen and unlabElse fields for a LabelIf object. *
														 ************************************************************/
				...
				while (j < obj.clauses.size()) {
					...
					nextStepNeedsLabel = InnerAddLabels(clause.unlabOr, false, // firstLabeled
							inWith, // inWith
							outAssigned, // inAssigned = {}
			...
			} else if (stmt.getClass()
					.equals(AST.WhileObj.getClass())) { /************************************************************
														 * This is a while statement. * - Add a label if needed and
														 * clear outAssigned. * - Call InnerAddLabels on the unlabDo
														 * clause. * - set nextStepNeedsLabel to false *
														 ************************************************************/
				 ...
				 ******************************************************/
				if (inWith) {
					throw new ParseDistAlgorithmException("`while' inside a `with' statement", stmt);
				}
				;
				NeedsLabel(stmt);
				hadOrAddedLabel = true;
				outAssigned.removeAllElements();
				Vector newOutAssigned = new Vector();
				InnerAddLabels(obj.unlabDo, false, // firstLabeled
						false, // inWith
						outAssigned, // inAssigned = {}
						newOutAssigned); // outAssigned
				nextStepNeedsLabel = false;
				/**********************************************************
				 * A label is never needed after a `while' statement * because that statement
				 * immediately follows the labeled * test. *
				 **********************************************************/
			} else if (stmt.getClass().equals(
					AST.WithObj.getClass())) { /************************************************************
												 * This is a with statement. Apply InnerAddLabels to the * Do field. If
												 * inWith = false, then this is an outermost * `with'. In that case,
												 * call with inAssigned = {}, and * add a label to the statement iff the
												 * variables assigned * within the `with' has nonempty intersection with
												 * the * inAssigned parameter of the current call. *
												 ************************************************************/
				...
				;
				Vector newOutAssigned = new Vector();
				nextStepNeedsLabel = InnerAddLabels(obj.Do, false, // firstLabeled
						true, // inWith
				...
				if (!inWith) {
					 ...
					 ********************************************************/
					if (!HasEmptyIntersection(inAssigned,
							outAssigned)) { /****************************************************
				...
				;
			} else if (stmt.getClass().equals(AST.AssignObj
					.getClass())) { /************************************************************
									 * This is an Assign statement. Set assignedVbls to the * set of variables being
									 * assigned. If this has non-empty * intersection with outAssigned, then
									 * statement needs a * label. Else add those variables to outAssigned. Set *
									 * nextStepNeedsLabel false, since there's no a priori * reason why an
									 * assignment needs a label after it. *
									 ************************************************************/
...
// DistPcalDebug.printVector(outAssigned, "out") ;
				/************************************************************
				...
				while (j < obj.ass.size()) {
					AST.SingleAssign assgn = (AST.SingleAssign) obj.ass.elementAt(j);
				...
				if (HasEmptyIntersection(outAssigned, assignedVbls)) {
					...
					if (inWith) {
						throw new ParseDistAlgorithmException(
								"Second assignment to same variable " + "inside a `with' statement", stmt);
					}
					;
				...
				nextStepNeedsLabel = false;
						 ...
						 * Some other statement type. Set nextStepNeedsLabel and * set assignedVbls to
						 * the set of variables being assigned * by this statement. Should set
						 * nextStepNeedsLabel to * true and set assignedVbls to a non-empty set iff this
						 * is * a call, return, callReturn, or callGoto. *
						 ************************************************************/
				nextStepNeedsLabel = false;
				Vector assignedVbls = new Vector();

				/************************************************************
				 * Set isCallOrReturn true iff this is a call, return, * callReturn, or
				 * callGoto. Will set setsPrcdVbls true iff * this is a return or callReturn or
				 * a call of * currentProcedure. *
				 ************************************************************/
					...
					if (setsPrcdVbls) {
						...
						while (j < procedures.size()) {
							...
							while (k < prcd.params.size()) {
								AST.PVarDecl decl = (AST.PVarDecl) prcd.params.elementAt(k);
							...
							while (k < prcd.decls.size()) {
								AST.PVarDecl decl = (AST.PVarDecl) prcd.decls.elementAt(k);
								assignedVbls.addElement(decl.var);
								k = k + 1;
							}
							;
				...
				if (HasEmptyIntersection(outAssigned, assignedVbls)) {
					...
					if (inWith) {
						throw new ParseDistAlgorithmException(
								"Call or return makes second assignment " + "to a variable inside a `with' statement",
								stmt);
					}
					;
					NeedsLabel(stmt);
	...
	/**********************************************************************
	 * Add a label to statement stmt if it doesn't already have one. The * label is
	 * DistPcalParams.LabelRoot concatenated with the next number * greater than or
	 * equal to nextLabelNum for which the resulting * label has not been typed by
	 * the user. *
	 **********************************************************************/
	{
		if (stmt.lbl.equals("")) {
			String lbl = DistPcalParams.LabelRoot + nextLabelNum;
			nextLabelNum = nextLabelNum + 1;
			while (allLabels.containsKey(lbl)) {
				lbl = DistPcalParams.LabelRoot + nextLabelNum;
				nextLabelNum = nextLabelNum + 1;
			}
			;
			stmt.lbl = lbl;
	...
	/***************************************************************************
	 * SETS OF STRINGS * * Methods for handling sets of strings, where such a set is
	 * represented * as a vector of its elements. *
	 ***************************************************************************/
	public static boolean IsIn(String elt, Vector set)
	/**********************************************************************
	 * True iff elt is in `set'. Implemented in a dumb way because it * should
	 * usually be called when it returns false. *
	 **********************************************************************/
	{
		int i = 0;
	...
	/**********************************************************************
	 * Returns the value of (S \cap T = {}). Implemented in a dumb way * because it
	 * should usually be called to return false. *
	 **********************************************************************/
	{
	...
	/**********************************************************************
	 * Sets the non-null vector `out' to a copy of the non-null vector * `in'. Note
	 * that `out' is a "call-by-reference" argument, while * `in' is a
	...
	public static Vector MakeLabeledStmtSeq(Vector stmtseq) throws ParseDistAlgorithmException
	/**********************************************************************
	 * Returns the sequence of <LabeledStmt> objects represented by the * sequence
	 * of <Stmt> objects and their lbl fields. *
	 **********************************************************************/
	{
	...
	/**********************************************************************
	 * This does the real work of MakeLabeledStmtSeq. It is made a * separate
	 ...
	 **********************************************************************/
	{
		...
		while (nextStmt < stmtseq.size()) {
			...
			DistPcalDebug.Assert(!stmt.lbl.equals(""), "Missing Label in MakeLabeledStmtSeq");
			...
			DistPCalLocation lstmtBegin = null;
			if (!stmt.lbl.equals("")) {
				lstmtBegin = stmt.lblLocation;
			}

			/****************************************************************
			 * lstmt.stmts is obtained from the sequence of <Stmt>s * consisting of stmt and
			 * all immediately following unlabeled * statements in stmtseq. *
			 ****************************************************************/
	...
	/**********************************************************************
	 * stmtseq is a sequence of statement objects that are produced by * GetStmtSeq.
	 * This procedure expands the substatements within each * LabelIf, LabelEither,
	 * and While object in stmtseq. For a LabelIf, * this removes the labeled
	 * statements from the unlabThen and * unlabElse sequences and puts them in
	 * labThen and labElse. *
	 **********************************************************************/
	{
		...
		while (i < stmtseq.size()) {
			...
			if (stmt.getClass().equals(AST.LabelIfObj
					.getClass())) { /************************************************************
									 * This is an if statement. Set the unlabThen field to the * result of calling
									 * FixStmtSeq on the prefix of its * current value consisting of unlabeled
									 * statements. Set * labThen to the sequence of labeled statement obtained by *
									 * calling InnerMakeLabeledStmtSeq on the rest of the * statement sequence. *
									 ************************************************************/
				AST.LabelIf obj = (AST.LabelIf) stmt;
				/******************************************************
				 * Sets obj to an alias of stmt of the right type. *
				...
				while ((curr.size() > 0) && ((AST) curr.elementAt(0)).lbl.equals("")) {
				...
				/********************************************************
				 * Set the unlabElse and labElse fields in an analogous * fashion. *
				 ********************************************************/
				curr = obj.unlabElse;
				...
				obj.labElse = InnerMakeLabeledStmtSeq(curr);
					...
					AST.LabelEitherObj.getClass())) { /************************************************************
														 * This is a LabelEither object. For each `or' clause, set * the
														 * unlabDo and labDo fields in the same way as the * unlabThen
														 * and labThen fields for a LabelIf object. *
														 ************************************************************/
				AST.LabelEither obj = (AST.LabelEither) stmt;
				/******************************************************
				 * Sets obj to an alias of stmt of the right type. *
				...
				;
					...
					AST.WhileObj.getClass())) { /********************************************************
												 * This is a while statement. Set the unlabDo field to * the prefix of
												 * its current value consisting of * unlabeled statements. Set labDo to
												 * the sequence of * labeled statement obtained by calling *
												 * InnerMakeLabeledStmtSeq on the rest of the statement * sequence. *
												 ********************************************************/
				AST.While obj = (AST.While) stmt;
				/******************************************************
				 * Sets obj to an alias of stmt of the right type. *
	...
	/***************************************************************************
	 * CHECKING AND FIXING THE AST * * Methods for checking the AST constructed from
	 * the simplified grammar, * converting <LabeledIf> nodes to <If> nodes where
	 * appropriate, and * checking that the AST obeys the actual grammar contained
	 * in the +CAL * document (so the rules for where labels can appear are
	 * satisfied).
	 * 
	 * @throws ParseDistAlgorithmException *
	 ***************************************************************************/
	public static void CheckLabeledStmtSeq(Vector stmtseq) throws ParseDistAlgorithmException {
		int i = 0;
		while (i < stmtseq.size()) {
	...
	public static int ClassifyStmtSeq(Vector stmtseq) throws ParseDistAlgorithmException
	/**********************************************************************
	 * Vector stmtseq must be a vector of <Stmt>s. This method returns * * 0 : If
	 * stmtseq contains no call, return, goto or labeled * statement nested within
	 * any of its statements. * * 1 : If stmtseq contains a call, return, or goto
	 * but no labeled * statement. * * 2 : If stmtseq contains a labeled statement.
	 * * * It checks that any calls, returns, gotos, or inner labeled * statements
	 * come where they belong, and it converts LabelIf and * LabelEither objects to
	 * If and Either objects where appropriate. *
	 **********************************************************************/
	{
		int i = 0;
		int result = 0;
		while (i < stmtseq.size()) {
			...
			if (node.getClass().equals(AST.LabelIfObj.getClass())) {
				...
				if (result < 2) {
					...
					while (j < eitherNode.clauses.size()) {
						newEither.ors.addElement(((AST.Clause) eitherNode.clauses.elementAt(j)).unlabOr);
						j = j + 1;
					}
					;
				...
				if (result == 2) {
					throw new ParseDistAlgorithmException(
							"with statement at " + node.location() + " contains a labeled statement");
				}
			} else if (node.getClass().equals(AST.WhileObj.getClass())) {
				if (i != 0) {
					DistPcalDebug.ReportBug(
							"ParseAlgorithm.ClassifyStmtSeq encountered `while'" + " not at beginning of StmtSeq.");
				}
				;
				...
				if (!((i < stmtseq.size() - 1)
						&& (stmtseq.elementAt(i + 1).getClass().equals(AST.ReturnObj.getClass())))) {
					result = 1;
				}
				;
			} else if ((node.getClass().equals(AST.ReturnObj.getClass()))
					|| (node.getClass().equals(AST.CallReturnObj.getClass()))) {
				if (currentProcedure == null) {
					throw new ParseDistAlgorithmException("return statement not in a procedure at " + node.location());
				}
				;
				result = 1;
			} else if (node.getClass().equals(AST.GotoObj.getClass())
					|| node.getClass().equals(AST.CallGotoObj.getClass())) {
				result = 1;
			} else if (!(node.getClass().equals(AST.AssignObj.getClass())
					|| node.getClass().equals(AST.WhenObj.getClass())
					|| node.getClass().equals(AST.PrintSObj.getClass())
					|| node.getClass().equals(AST.AssertObj.getClass())
					|| node.getClass().equals(AST.SkipObj.getClass())
					|| node.getClass().equals(AST.MacroCallObj.getClass())
					
					//why isn't it already here
					|| node.getClass().equals(AST.SingleAssignObj.getClass())
					|| node.getClass().equals(AST.ChannelSenderObj.getClass())
					|| node.getClass().equals(AST.ChannelReceiverObj.getClass()))) {
				DistPcalDebug.ReportBug("ParseAlgorithm.ClassifyStmtSeq encountered the unexpected" + " statement type "
			...
			if (result > 0) {
				if (i == stmtseq.size() - 1) {
					...
					DistPcalDebug.ReportBug("Translator discovered later than it should have " + " that\n  "
	...
	public static int ClassifyIf(AST.LabelIf node) throws ParseDistAlgorithmException
	/**********************************************************************
	 * Checks a LabelIf node and returns the kind of nonterminal of the * real BNF
	 * grammar with the following encoding it should really be, * as follows * * 0 :
	 * <If> * 1 : <FinalIf> * 2 : <LabelIf> * * It is a <LabelIf> iff it contains a
	 * labeled statement somewhere * within it (possibly nested inside another <If>.
	 * *
	 **********************************************************************/
	{ /********************************************************************
		 * Do checking in order that finds first error in the algorithm * first. *
		 ********************************************************************/
	...
	public static int ClassifyEither(AST.LabelEither node) throws ParseDistAlgorithmException
	/**********************************************************************
	 * Checks a LabelEither node and returns the kind of nonterminal of * the real
	 * BNF grammar with the following encoding it should really * be, as follows * *
	 * 0 : <Either> * 1 : <FinalEither> * 2 : <LabelEither> * * It is a
	 * <LabelEither> iff it contains a labeled statement * somewhere within it
	 * (possibly nested inside another <Either>. *
	 **********************************************************************/
	{ /********************************************************************
		 * Do checking in order that finds first error in the algorithm * first. *
		 ********************************************************************/
	...
	public static void CheckForDuplicateMacros(Vector macros) throws ParseDistAlgorithmException
	/**********************************************************************
		...
		while (i < macros.size()) {
			...
			while (j < macros.size()) {
				if (namei.equals(((AST.Macro) macros.elementAt(j)).name)) {
					throw new ParseDistAlgorithmException("Multiple definitions of macro name `" + namei + "'");
				}
				j = j + 1;
			}
			i = i + 1;
...
//                        Vector macros) throws ParseDistAlgorithmException  // of Macro
//     /**********************************************************************
//     * Expands macro calls.                                                *
//     **********************************************************************/
//     { int i = 0 ;
...
//     }
	...
	public static void ExpandMacrosInStmtSeq(Vector stmtseq, Vector macros) throws ParseDistAlgorithmException

	{
		...
		while (i < stmtseq.size()) {
			...
			if (stmt.getClass().equals(AST.LabelIfObj.getClass())) {
				ExpandMacrosInStmtSeq(((AST.LabelIf) stmt).unlabThen, macros);
				ExpandMacrosInStmtSeq(((AST.LabelIf) stmt).unlabElse, macros);
				...
				while (j < eNode.clauses.size()) {
					ExpandMacrosInStmtSeq(((AST.Clause) eNode.clauses.elementAt(j)).unlabOr, macros);
					j = j + 1;
				}
				;
			} else if (stmt.getClass().equals(AST.WithObj.getClass())) {
				ExpandMacrosInStmtSeq(((AST.With) stmt).Do, macros);
			} else if (stmt.getClass().equals(AST.WhileObj.getClass())) {
				ExpandMacrosInStmtSeq(((AST.While) stmt).unlabDo, macros);
			} else if (stmt.getClass().equals(AST.MacroCallObj.getClass())) {
				Vector expansion = ExpandMacroCall(((AST.MacroCall) stmt), macros);
	...
	public static Vector ExpandMacroCall(AST.MacroCall call, Vector macros) throws ParseDistAlgorithmException { // Set
																													// macroDef
																													// to
																													// the
																													// Macro
		...
		if (macroDef == null) {
			throw new ParseDistAlgorithmException("Macro " + call.name +
		...
		if (macroDef.params.size() != numOfArgs) {
			throw new ParseDistAlgorithmException(
					"Macro " + call.name + " called with wrong number of arguments at " + "\n    " + call.location());
		}
		;
		Vector result = SubstituteInStmtSeq(macroDef.body, call.args, macroDef.params, call.line, call.col);
		/*
		 * Copy lbl and lblOrigin fields of call to first statement of expansion. Set
		 * the macroOriginBegin and macroOriginEnd of the first and last statements.
		 * Note that either (or both) of those fields could already be set if the
		 * statement arose from the expansion of a macro in the current macro. However,
		 * since the PcalTLAGen.GenLabeledStmt method wants those fields set from the
		 * macro call in the main body of the PlusCal algorithm, which is the last one
		 * expanded in a sequence of macros calling macros.
		 */
		if (result.size() > 0) {
	...
	public static Vector SubstituteInLabeledStmtSeq(Vector stmts, // of AST.LabeledStmt
			Vector args, // of TLAExpr
			Vector params) throws ParseDistAlgorithmException // of String
	{
		Vector result = new Vector();
		int i = 0;
		while (i < stmts.size()) {
			result.addElement(SubstituteInLabeledStmt((AST.LabeledStmt) stmts.elementAt(i), args, params));
			i = i + 1;
		}
		return result;
	}

	public static AST.LabeledStmt SubstituteInLabeledStmt(AST.LabeledStmt stmt, Vector args, // of TLAExpr
			Vector params) throws ParseDistAlgorithmException {
		AST.LabeledStmt result = new AST.LabeledStmt();
	 ...
	 * statements found in the body of a macro. However, it is simple to * define a
	 * SubstituteInLabeledStmtSeq method and enhance * SubstituteInStmtSeq so the
	 * SubstituteInLabeledStmtSeq can be used to do * arbitrary substitutions in the
	 * body of a process or uniprocess * algorithm. *
	 ***************************************************************************/
	public static Vector SubstituteInStmtSeq(Vector stmts, // of AST
			Vector args, // of TLAExpr
			Vector params, // of String
			int macroLine, int macroCol) throws ParseDistAlgorithmException
	/*********************************************************************
	 * A vector of new AST nodes obtained from the statements in stmts * by
	 * substituting the expressions in args for the corresponding * parameters in
	 * params inside the expansion of a macro call at line * macroLine, column
	 * macroCol, where macroLine = -1 if this is not * being called because of a
	 * macro expansion. *
	 *********************************************************************/
	{
		Vector result = new Vector();
	...
	public static AST SubstituteInStmt(AST stmt, Vector args, // of TLAExpr
			Vector params, // of String
			int macroLine, int macroCol) throws ParseDistAlgorithmException
	/*********************************************************************
	 * A new AST node obtained from statement stmt by substituting the * expressions
	 * in args for the corresponding parameters in params * inside the expansion of
	 * a macro call at line macroLine, column * macroCol, where macroLine = -1 if
	 * this is not being called during * macro expansion. * * Note that the origin
	 * of the new node should be the same as that of * the original. *
	 *********************************************************************/
	{
		/*******************************************************************
		 * The following statements are ones that may appear in a macro * definition
		 * body. *
		 *******************************************************************/
		try {
			if (stmt.getClass().equals(AST.AssignObj.getClass())) {
				...
				while (i < tstmt.ass.size()) {
					result.ass.addElement(SubstituteInSingleAssign((AST.SingleAssign) tstmt.ass.elementAt(i), args,
							params, macroLine, macroCol));
					i = i + 1;
				}
				;
				return result;
			...
			if (stmt.getClass().equals(AST.IfObj.getClass())) {
				...
				result.Then = SubstituteInStmtSeq(tstmt.Then, args, params, macroLine, macroCol);

				result.Else = SubstituteInStmtSeq(tstmt.Else, args, params, macroLine, macroCol);
			...
			if (stmt.getClass().equals(AST.EitherObj.getClass())) {
				...
				while (i < tstmt.ors.size()) {
					result.ors.addElement(
							SubstituteInStmtSeq((Vector) tstmt.ors.elementAt(i), args, params, macroLine, macroCol));
					i = i + 1;
				}
				;
			...
			if (stmt.getClass().equals(AST.WithObj.getClass())) {
				...
				result.Do = SubstituteInStmtSeq(tstmt.Do, args, params, macroLine, macroCol);
				return result;
			}
			;
			...
			/*******************************************************************
			 * The following statements are ones that may not appear in a macro * definition
			 * body. *
			 *******************************************************************/
			if (stmt.getClass().equals(AST.WhileObj.getClass())) {
				...
				result.test = tstmt.test.cloneAndNormalize();
				result.test.substituteForAll(args, params);
				result.unlabDo = SubstituteInStmtSeq(tstmt.unlabDo, args, params, macroLine, macroCol);
				result.labDo = SubstituteInLabeledStmtSeq(tstmt.labDo, args, params);
			...
			if (stmt.getClass().equals(AST.LabelIfObj.getClass())) {
				...
				result.test = tstmt.test.cloneAndNormalize();
				result.test.substituteForAll(args, params);
				result.unlabThen = SubstituteInStmtSeq(tstmt.unlabThen, args, params, macroLine, macroCol);
				result.labThen = SubstituteInLabeledStmtSeq(tstmt.labThen, args, params);
				result.unlabElse = SubstituteInStmtSeq(tstmt.unlabElse, args, params, macroLine, macroCol);
				result.labElse = SubstituteInLabeledStmtSeq(tstmt.labElse, args, params);
			...
			if (stmt.getClass().equals(AST.LabelEitherObj.getClass())) {
				...
				while (i < tstmt.clauses.size()) {
					...
					newClause.labOr = SubstituteInLabeledStmtSeq(oldClause.labOr, args, params);
					newClause.unlabOr = SubstituteInStmtSeq(oldClause.unlabOr, args, params, macroLine, macroCol);
	...
	public static AST SubstituteInSingleAssign(AST.SingleAssign assgn, Vector args, // of TLAExpr
			Vector params, // of String
			int macroLine, int macroCol) throws ParseDistAlgorithmException
	/*********************************************************************
	 * A new AST.SingleAssign node obtained from assgn by substituting * the
	 * expressions in args for the corresponding parameters in * params inside the
	 * expansion of a macro call at line macroLine, * column macroCol, where
	 * macroLine = -1 if this is not being called * during macro expansion. *
	 *********************************************************************/
	{
		try {
			AST.SingleAssign result = new AST.SingleAssign();
			...
			/*******************************************************************
			 * If there is a subscript on the left-hand-side, substitute in * it. *
			 *******************************************************************/
			if ((assgn.lhs.sub.tokens != null) && (assgn.lhs.sub.tokens.size() != 0))
			/***************************************************************
			 * I'm not sure any more if an AST.SingleAssign representing * an assignment
			 * with no subscript has a null sub field or a * vector of length 0. *
			 ***************************************************************/
			{
			...
			if (found) {
				...
				if (varToken.type != TLAToken.IDENT) {
					throw new ParseDistAlgorithmException("Macro expansion substitutes `" + varToken.string
							+ "' for assignment variable\n    at " + result.location());
				}
				;
				result.lhs.var = varToken.string;

				if (subForVar.stepCoord(new IntPair(0, 0),
						1) != null) { /***********************************************************
										 * More than a single token is being substituted for the * variable--for
										 * example, we may be substituting "x[0]" * for "y" in "y[i] := ...". The rest
										 * of the expression * must therefore be prepended to the subscript. * * We
										 * first set exprCopy to a clone of the substituting * expression with the first
										 * token (the one that became * the assignment variable) removed. *
										 ***********************************************************/
					...
					exprCopy.normalize();
					/*********************************************************
					 * We must normalize because the deleted token may be an * anchor token, and the
					 * deletion might have created an * empty first line. *
					 *********************************************************/
					...
					{ /*******************************************************
						...
						result.lhs.sub = exprCopy;
								 ...
								 * There was already a subscript, so we must prepend. * We don't add any space
								 * after the prepended * expression because, to be sensible, the existing *
...
//      { return  }
	...
	private static void ParsingError(String msg) throws ParseDistAlgorithmException {
		throw new ParseDistAlgorithmException(msg + "\n    line " + lastTokLine + ", column " + lastTokCol);
	}

	public static void GobbleCommaOrSemicolon() throws ParseDistAlgorithmException
	/**********************************************************************
	...
	public static void GobbleBeginOrLeftBrace() throws ParseDistAlgorithmException
	/**********************************************************************
	 * Used when expecting a "begin" in the p-syntax or a "{" in the * c-syntax.
	 * Gobbles is and sets pSyntax or cSyntax. *
	 **********************************************************************/
	{
		if (pSyntax) {
			GobbleThis("begin");
			...
			DistPcalDebug.ReportBug("Syntax not initialized.");
		}
		;
		return;
	}

	public static void GobbleEndOrRightBrace(String str) throws ParseDistAlgorithmException
	/**********************************************************************
	 * Used when expecting an "end str" in the p-syntax or a "}" in the * c-syntax.
	 * *
	 **********************************************************************/
	{
		if (pSyntax) {
			...
			DistPcalDebug.ReportBug("Bad call of GobbleEndRightBrace");
		}
		;
		return;
	}

	public static void GobbleThis(String str) throws ParseDistAlgorithmException {
		/********************************************************************
		 * If the next token is not str, then report an error. Otherwise, * just move
		 * past the token. However, if str is a semicolon and * the next token indicates
		 * that the input is missing an obviously * unnecessary semicolon, then don't
		 * report an error--for example, * if the next token is "end". If the missing
		 * semicolon should * really be there--for example, if the user typed "x : = x +
		 * 1 * print x", then print a warning message about the missing * semicolon. The
		 * missing semicolon is innocuous if the next token * is * * end begin else
		 * elsif or macro procedure process define * * in the p-syntax or "{" or "}" in
		 * the c-syntax. Since GobbleThis * can be called before we know which syntax is
		 * used, it ignores * the missing semicolon in either case. * * It used to
		 * produce a warning if the next token is * * if either while with call return
		 * goto print assert * skip * * However, in this case the translation could be
		 * incorrect because * the missing ";" could have caused a label to be
		 * incorporated as * part of the previous statement. So, it produces a missing
		 * ";" * error. The case is still tested for and a different error * message
		 * produced that might be more helpful. *
		 ********************************************************************/
		if (str.equals(";")) {
			String nxt = PeekAtAlgToken(1);
			if (nxt.equals("end") || nxt.equals("begin") || nxt.equals("else") || nxt.equals("elsif")
					|| nxt.equals("or") || nxt.equals("do") || nxt.equals("macro") || nxt.equals("procedure")
					|| nxt.equals("process") || nxt.equals("fair") || nxt.equals("define") || nxt.equals("}")
					|| nxt.equals("{") || nxt.equals("node")) {
				return;
			}
			;
			if (nxt.equals("if") || nxt.equals("either") || nxt.equals("while") || nxt.equals("with")
					|| nxt.equals("call") || nxt.equals("return") || nxt.equals("goto") || nxt.equals("print")
					|| nxt.equals("assert")
					|| nxt.equals("skip")) { /************************************************************
												 * This was changed from a warning to an error by LL on 28 * Feb 2006
												 * because the translation will be incorrect if * there is a label
												 * following the missing ";". *
												 ************************************************************/
				throw new ParseDistAlgorithmException(
						"Missing `;' before line " + (curTokLine[0] + 1) + ", column " + (curTokCol[0] + 1));
			}
			;
		}
		;
	...
	public static void MustGobbleThis(final String str) throws ParseDistAlgorithmException {
		final String tok;
		try {
			tok = GetAlgToken();
		} catch (ParseDistAlgorithmException e) {
	...
	public static boolean GobbleEqualOrIf() throws ParseDistAlgorithmException
	/**********************************************************************
	 * Gobbles the next token, returns true if it is "=", false if it is * "\in",
	 ...
	 **********************************************************************/
	{
		...
		ParsingError("Expected \"=\" or \"\\in\"  but found \"" + tok + "\"");
	...
	/**********************************************************************
	 * LAT[0], LAT[1], ... , LAT[LATsize-1] contain tokens that have * been read
	 * from the algorithm with Tokenize.GetAlgorithmToken but * not yet processed.
	 * It should never contain a token that's part of * an Expr. *
	 **********************************************************************/
	 ...
	 * PcalCharReader.getLineNumber() and * PcalCharReader.getColumnNumber() when
	 * the reader was positioned * just before the token in LAT[i]. *
	 **********************************************************************/

	public static int lastTokCol;
	public static int lastTokLine;
	/*********************************************************************
	 * The column and line number of the last token returned with * GetAlgToken,
	 * where the numbering starts at 1. The translation * from Java ordinals to
	 * human ordinals occurs when these variables * are set. *
	 * 
	 * @throws ParseDistAlgorithmException * *
	 **********************************************************************/
	/**
	...
	/**
	 * Returns the PCalLocation object corresponding to the beginning of the last
	 * token returned by GetAlgToken or gobbled by a Gobble... method.
	 * 
	 * @return
	 */
	private static DistPCalLocation GetLastLocationStart() {
		return new DistPCalLocation(lastTokLine - 1, lastTokCol - 1);
	}

	/**
	 * Returns the PCalLocation object corresponding to the position to the right of
	 * the last token returned by GetAlgToken or gobbled by a Gobble... method.
	 * 
	 * @return
	 */
	private static DistPCalLocation GetLastLocationEnd() {
		return new DistPCalLocation(lastTokLine - 1, lastTokCol - 1 + lastTokString.length());
	}

	public static String GetAlgToken() throws ParseDistAlgorithmException
	/**********************************************************************
	 ...
	 **********************************************************************/
	{
		if (LATsize == 0) {
			charReader.peek();
			/***************************************************************************
			 * The following bug should have been fixed by the addition of the * Uncomment
			 * method by LL on 18 Feb 2006. * * BUG * * This is the source of the
			 * position-reporting bug. If there's a comment * between the current position
			 * and the beginning of the next non-comment * token, then charReader.peek()
			 * will position the reader at the beginnig * of the comment. * * One fix is to
			 * implement a peek() method that skips over comments. Such * a method needs to
			 * be implemented in the Tokenize class. However, that * requires modifying the
			 * actual tokenizing code, whic requires * understanding it. A better fix might
			 * be to move LAT and LATsize to the * Tokenize class, so both GetAlgorithmToken
			 * and TokenizeExpr get their * tokens from it. This would permit a real
			 * PeekAtAlgToken method to be * implemented in the Tokenize class that can be
			 * called to look * arbitrarily far ahead in the token stream without
			 * interfering with the * tokenizing of expressions. The PcalCharReader's kludgy
			 * "peek" method * would then not be needed. However, this fix requires
			 * modifying a fair * amount of code. *
			 ***************************************************************************/
	...
	public static String PeekAtAlgToken(int tokNum) throws ParseDistAlgorithmException
	/**********************************************************************
	 * This returns the tokNum-th token ahead, but does not actually * remove any
	 * token from the input stream. PeekAtAlgToken(1) returns * the next token,
	 * PeekAtAlgToken(2) returns the one after that, etc. *
	 **********************************************************************/
	{
		while ((LATsize < tokNum)) {
			/****************************************************************
			 * Move charReader to beginning of next non-space token and get * line and
			 * column numbers. *
			 ****************************************************************/

			/****************************************************************
			 * Change made 14 January 2009 by LL. Changed call of * charReader.peek() to
			 * this call plus test for end of file to * correct bug_08_12_12. *
			 ****************************************************************/
			if (charReader.peek().equals("\t")) {
				throw new ParseDistAlgorithmException(
						"Premature end of file, perhaps because of " + "unclosed comment, near\n" + "    line "
								+ (curTokLine[LATsize] + 1) + ", column " + (curTokCol[LATsize] + 1));
			}
			;
	...
	public static String PeekPastAlgToken(int tokNum) throws ParseDistAlgorithmException
	/**********************************************************************
	 * This performs and returns the results of a PcalCharReader.peek() * at the
	 * input stream after the tokNum-th token after the current * one, where tokNum
	 * = 0 means from the current logical point in the * input stream. This produces
	 * an error if tokNum > LATsize, meaning * that you can't perform this operation
	 * at a point before a token * that has been peeked at using PeekAtAlgToken. *
	 **********************************************************************/
	{
		if (tokNum < LATsize) {
			DistPcalDebug.ReportBug("ParseAlgorithm.PeekPastAlgToken called " + "to peek after a token"
	 ...
	 * kludgery was confused * by comments. The simple solution was to remove
	 * comments before * parsing. The Uncomment method does this. It is called on a
	 * copy of * the input used only to create the abstract syntax tree, so it
	 * doesn't * matter what it does to anything outside the algorithm. Therefore,
	 * the * method doesn't bother to try to find the actual end of the algorithm. *
	 * It stops the processing if it finds an unmatched *), since it then * should
	 * be outside the algorithm. * * Bug fixed by LL on 28 Feb 2006
	 ***************************************************************************/
	public static void uncomment(Vector inp, int begLine, int begCol) throws ParseDistAlgorithmException

	/**********************************************************************
	 * Replace all (* *) comments by spaces, and delete all \* comments, * in the
	 * string vector inp, starting at line begLine and column * begCol, and ending
	 * with the first unmatched *) . * * Note: The proper handling of \* with
	 * respect to (* *) is indicated * by the observation that both * * \* (* xxx *
	 * * and * * (* \* xx *) * * are complete comments. *
	 **********************************************************************/
	{
		int line = begLine;
		int col = begCol;
		...
		while (notDone && line < inp.size()) {
			...
			while (notDone && col < oldLine.length()) {
				...
				if ((inChar == '(') && !inString && (col < oldLine.length() - 1) && (oldLine.charAt(col + 1) == '*')) {
				...
				} else if ((inChar == '*') && !inString && (col < oldLine.length() - 1)
					...
					{
						if ((inChar == '\\') && (col < oldLine.length() - 1) && (oldLine.charAt(col + 1) == '*')) { // End-of-line
																													// comment
							outChar = ' ';
							col = oldLine.length();
						} else if (inChar == '"') {
			...
			;
			if (inString) {
				throw new ParseDistAlgorithmException("Unterminated string in line " + (line + 1));
			}
	...
	/***************************************************************************
	 * ARGUMENTS OF METHODS FOR PROCESSING THE pcal INPUT * * Many of the following
	 * methods for processing the pcal input start at a * certain point and process
	 * a region of the input. Those for which the * region can extend across
	 * multiple lines have the following arguments. * (An arguments will not appear
	 * if it can have only one possible value * or is not needed.) * * StringVector
	 * inputVec * The pcal input file, usually with tabs removed. * * IntPair curLoc
	 * * The <line, column> location (in Java coordinates) of the beginning * of the
	 * region to process. This object is modified by the method * to leave it
	 * pointing immediately after the last character of the * region that was
	 * processed. If that character ends a line, then * it points to the nonexistent
	 * character after the last character * on the line. (That is, the value of
	 * curLoc does not point to * the first character on the next line.) * * String
	 * errorMsg * Used only for a method that has multiple uses and can throw a *
	 * ParseDistAlgorithmException. It is the error reported if the * exception is
	 * thrown. * * Some methods just process a single line of the input. Those have
	 * the * following arguments: * * - A string that is the current line. * * - An
	 * int that is the position within the line of the beginning of * the region. *
	...
	/***
	 * If curLoc points to a version statement, this method processes the statement.
	 * It is a no-op otherwise. It calls DistPcalParams.ProcessVersion to do the
	 * actual processing, and writes the statement to the tla output file. There is
	 * no comment removal. The method returns false iff there is an error,calling
	 * DistPcalDebug.reportError to report the error.
	 * 
	 * See the comments above for an explanation of the arguments.
	 */
	/***************************
	 * public static boolean ProcessVersion(Vector inputVec, IntPair curLoc) {
	 * String curLine = GotoNextNonSpace(inputVec, curLoc); boolean error = false;
	 * if (curLine.substring(curLoc.two).startsWith("version")) { // Process the
	 * version number and move curLoc and curLine // to just after the closing ")".
	 * curLoc.two = curLoc.two + 7; // go to position after "version" curLine =
	 * GotoNextNonSpace(inputVec, curLoc); if
	 * (!curLine.substring(curLoc.two).startsWith("(")) { curLoc.one =
	 * inputVec.size(); error = true; } curLoc.two ++; curLine =
	 * GotoNextNonSpace(inputVec, curLoc); int endOfArg = NextDelimiterCol(curLine,
	 * curLoc.two); if (!DistPcalParams.ProcessVersion(curLine.substring(curLoc.two,
	 * endOfArg))){ curLoc.one = inputVec.size(); error = true; } curLoc.two =
	 * endOfArg; curLine = GotoNextNonSpace(inputVec, curLoc); if (!error &&
	 * !curLine.substring(curLoc.two).startsWith(")")) { curLoc.one =
	 * inputVec.size(); error = true; } curLoc.two ++; // curLine =
	 * GotoNextNonSpace(untabInputVec, outputVec, curLoc); } if (error) {
	 * DistPcalDebug.reportError("Error in version statement"); return false; }
	 * return true ; }
	 ************************/

	/***
	 * If the next non-space character is the beginning of the "options" string of
	 * an options statement, then this method processes the statement. Otherwise, it
	 * does nothing. The actual processing of the arguments is done by calling
	 ...
	 * The options statement is written to the tla output. There is no removal of
	 * comments.
	 * 
	 * See the comments above for an explanation of the arguments.
	 */
	public static int ProcessOptions(Vector untabInputVec, IntPair curLoc) {
		...
		if (curLine.substring(curLoc.two).startsWith("options")) {
			...
			if (!curLine.substring(curLoc.two).startsWith("(")) {
				...
				DistPcalDebug.reportError("`PlusCal options' not followed by '('");
			...
			if (!(curLoc.one < untabInputVec.size())) {
				DistPcalDebug.reportError("No closing ')' found in options statement");
				return trans.STATUS_EXIT_WITH_ERRORS;
			}
			curLoc.two++;
			curLine = GotoNextNonSpace(untabInputVec, curLoc);
...
// printArray(argsArray);           
	 ...
	 * Otherwise, it throws a ParseDistAlgorithmException. The token must be a
	 * sequence of letters, numbers, and "_" characters that is terminated by any
	 * other kind of character or the end of a line.
	 * 
	 * See the comments above for an explanation of the arguments.
	 * 
	 ...
	 * @throws ParseDistAlgorithmException
	 */
	public static void FindToken(String token, Vector inputVec, IntPair curLoc, String errorMsg // The error message to
																								// be reported if not
																								// found.
	) throws ParseDistAlgorithmException {
		boolean found = false;
		while ((!found) && (curLoc.one < inputVec.size())) {
			...
			if (curLine.substring(curLoc.two).startsWith(token)) {
				...
				if ((endLoc >= curLine.length())
						|| !(Character.isLetter(curLine.charAt(endLoc)) || (curLine.charAt(endLoc) == '_'))) {
					found = true;
...
//                   }
			...
			// if ((c == '(') && (curLoc.two + 1 < curLine.length()) &&
		...
		if (!found) {
			throw new ParseDistAlgorithmException(errorMsg);
		}
		;
	...
	/**
	 * Assumes that curLoc points to a left brace ("{") in the String Vector
	 * inputVec, and it searches for the matching right brace. It updates curLoc to
	 * the position immediately after the token if it's found. Otherwise, it raises
	 * a ParseDistAlgorithmException.
	 * 
	 * See the comments above for an explanation of the arguments. The PlusCal code
	 * currently calls this only with replace = true and outputVec = null.
	 * 
	 * @param inputVec  Input String Vector
	 ...
	 * @throws ParseDistAlgorithmException
	 */
	public static void FindMatchingBrace(Vector inputVec, // Vector of strings
...
//                           // be replaced by spaces.
				...
				if (curLoc.two < curLine.length()) {
					...
					if (c == '}') {
						...
						curLine = (String) inputVec.elementAt(curLoc.one);
							...
							&& (curLine.charAt(curLoc.two + 1) == '*')) {
						ParseDistAlgorithm.gotoEndOfComment(inputVec, curLoc);
						curLine = (String) inputVec.elementAt(curLoc.one);
					} else if ((c == '\\') && (curLoc.two + 1 < curLine.length())
							&& (curLine.charAt(curLoc.two + 1) == '*')) {
						// if (replace) {
						...
						curLoc.two = ParseDistAlgorithm.findEndOfString(curLine, curLoc.two, curLoc.one);
					}
				}
	...
	/***********************************************************************
	 * Below are a bunch of methods called Next... that are implemented * using the
	 * String class's split() method. Since there's no * specification of that
	 * method, this is dangerous. The one trap I've * found so far is this:
	 * experimentation suggests that if the regexp * identifies the entire string as
	 * being composed of splitting * strings, then it returns an array of length 0.
	 * Who knows what * other features are lurking. I thought of using the following
...
//   }
	/**
	 * Returns the position of the first space at or after position col in str, or
	 * str.size() if there is none.
	 */
...
//   }
	...
	/**
	 * Returns the position of the first space , ",", or ")" at or after position
	 * col in str, or str.size() if there is none.
	 */
	...
	/**
	 * Returns the position of the first spaceat or after position col in str, or
	 * str.size() if there is none.
	 * 
	 * @param str
	 * @param col
	...
	/**
	 * Returns the position of the first "{", "}", quote ("), "(*", or "\*" at or
	 * after position col in str, or str.size() if there is none.
	 * 
	 * @param str
	 * @param col
	 * @return
	...
	/**
	 * Returns the position of the first character not a letter, number, or "_" at
	 * or after position col in str, or str.size() if there is none.
	 * 
	 * @param str
	 * @param col
	 * @return
	 ...
	 * location. If there is no non-space character found, then the location is set
	 * to column 0 of the first row after the end if inputVec. It returns the line
	 * to which curLoc is pointing to, or "" if loc is after the end of the Note: it
	 * does the right thing if loc is the column after the end of a row.
	 * 
...
//   }
	...
	/**
	 * This method assumes that str is a string whose character at position loc is a
	 * quote ("). It returns the position immediately after the matching quote that
	 * ends a TLA+ string. If there is no matching quote, then it throws a
	 * ParseDistAlgorithmException, reporting the error at line line+1, column loc+1
	 */
	public static int findEndOfString(String str, int loc, int line) throws ParseDistAlgorithmException {
		int pos = loc + 1;
		boolean found = false;
		...
		if (!found) {
			throw new ParseDistAlgorithmException(
					"Unterminated string begun at line " + "\n    line " + (line + 1) + ", column " + (loc + 1));
		}
		return pos;
	}

	/**
	 * Finds the end of a comment in the String Vector inputVec, assuming that loc
	 * is the location of the "(" in the "(*" that begins the comment, and advances
	 * loc to the position just beyond the end of the closing "*)".
	 * 
	 * See the comments above for an explanation of the arguments.
	 */
...
//      }
		if (!found) {
			throw new ParseDistAlgorithmException("Unterminated comment begun at line " + "\n    line "
					+ (curLoc.one + 1) + ", column " + (curLoc.two + 1));
		}
		return;
	}
