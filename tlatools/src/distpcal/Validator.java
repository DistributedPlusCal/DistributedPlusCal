...
public class Validator {
	...
	static final Pattern DISTPCAL_CHECKSUM_PATTERN = Pattern.compile(DistPcalParams.DISTPCAL_CHECKSUM_KEYWORD + "[0-9a-f]+$");
	static final Pattern TRANSLATED_DISTPCAL_CHECKSUM_PATTERN
									= Pattern.compile(DistPcalParams.TRANSLATED_DISTPCAL_CHECKSUM_KEYWORD + "[0-9a-f]+$");
	 ...
	 */
	private static final Pattern DISTPLUSCAL_OPTIONS = Pattern.compile("^[\\s]*DistPlusCal[\\s]+options",
		...
		if (PRE_TRIM_VALIDATION_CONTENT) {
			for (int i = 0; i < lines.length; i++) {
				...
				if (m.find()) {
					...
					final Matcher m2 = DISTPLUSCAL_OPTIONS.matcher(lines[i]);
					if (m2.find()) {
						startLine = i;
						break;
					}
	...
	private static ValidationResult validate(final List<String> specificationText) {
		...
		while (notDone) {
			try {
				 ...
				 * 		the character-by-character marching done in the ParseDistAlgorithm code versus using a
				 * 		regex matcher to split apart lines.
				 */
                ParseDistAlgorithm.FindToken("PlusCal", deTabbedSpecification, searchLoc, "");
                final String line = ParseDistAlgorithm.GotoNextNonSpace(deTabbedSpecification, searchLoc);
				...
				if (restOfLine.startsWith("options")) {
					...
					if (ParseDistAlgorithm.NextNonIdChar(restOfLine, 0) == 7) {
                        ...
                        ParseDistAlgorithm.ProcessOptions(deTabbedSpecification, searchLoc);
                        notDone = false;
                    }
                }
		...
		final int translationLine = trans.findTokenPair(deTabbedSpecification, 0,
														DistPcalParams.BeginXlation1, DistPcalParams.BeginXlation2);
		...
		if (translationLine == -1) {
			...
			final int endTranslationLine = trans.findTokenPair(deTabbedSpecification, translationLine + 1,
															   DistPcalParams.EndXlation1, DistPcalParams.EndXlation2);
			if (endTranslationLine == -1) {
                return ValidationResult.NO_TRANSLATION_EXISTS;
            }

			final String beginLine = deTabbedSpecification.get(translationLine);
        	Matcher m = Validator.DISTPCAL_CHECKSUM_PATTERN.matcher(beginLine);
        	if (m.find()) {
        		pcalMD5 = beginLine.substring(m.start() + DistPcalParams.DISTPCAL_CHECKSUM_KEYWORD.length());
        	} else {
        		return ValidationResult.NO_CHECKSUMS_EXIST;
        	}
        	final String endLine = deTabbedSpecification.get(endTranslationLine);
        	m = Validator.TRANSLATED_DISTPCAL_CHECKSUM_PATTERN.matcher(endLine);
        	if (m.find()) {
        		translatedMD5 = endLine.substring(m.start() + DistPcalParams.TRANSLATED_DISTPCAL_CHECKSUM_KEYWORD.length());
		...
		try {
			ParseDistAlgorithm.uncomment(deTabbedSpecification, algLine, algCol);
		} catch (ParseDistAlgorithmException e) {
		...
		//	then the ParseDistAlgorithm won't pick up the values..
		...
		final DistPcalCharReader reader = new DistPcalCharReader(deTabbedSpecification, algLine, algCol,
    ...
    static String calculateMD5(final String string) {
    	try {
    		...
    		DistPcalDebug.reportError("Unable to calculate MD5: " + e.getMessage());
    		return null;
    	}
    }
}
