...
public class DistPcalTranslate {

    private static DistPcalSymTab st = null;  /* Set by invocation of Explode */
    ...
    *********************************************************************/
    ...
    public static AST.Assign MakeAssign(String id, TLAExpr exp, Integer threadIndex) {
        /*********************************************************************
    ...
    private static AST.When CheckPC (String label, Integer threadIndex) {
        /*********************************************************************
    ...
    private static AST.Assign UpdatePC (String next, Integer threadIndex) {
        /*********************************************************************
        * Generate pc := next ;                                              *
        *********************************************************************/
...
//    }
    ...
    public static AST Explode (AST ast, DistPcalSymTab symtab) throws DistPcalTranslateException {
        /*********************************************************************
...
//            return ExplodeUniprocthreadIndexess((AST.Uniprocess) ast);
//        else 
    ...
    private static AST.Uniprocess ExplodeUniprocess (AST.Uniprocess ast, Integer threadIndex) throws DistPcalTranslateException {
        /*********************************************************************
...
//                ? st.UseThis(PcalSymTab.LABEL, "Done", "")
    ...
    private static AST.MultiNodes ExplodeMultiNodes (AST.MultiNodes ast) throws DistPcalTranslateException {
        /*********************************************************************
        ...
        *********************************************************************/
        ...
        i = 0;
        newast.nodes = new Vector(ast.nodes.size(), 10);
        while (i < ast.nodes.size()) {
        	
        	newast.nodes.addElement(ExplodeNode((AST.Node) ast.nodes.elementAt(i)));
            i = i + 1;
        }
    ...
    private static AST ExplodeProcedure (AST.Procedure ast) throws DistPcalTranslateException {
        /*********************************************************************
        ...
        *********************************************************************/
        ...
        while (i < ast.body.size()) {
            String next = (nextLS == null)
                ? st.UseThis(DistPcalSymTab.LABEL, "Error", "")
                : nextLS.label;
    ...
    private static AST ExplodeNode(AST.Node ast) throws DistPcalTranslateException {
        /*********************************************************************
    ...
    private static Vector CopyAndExplodeLastStmt(Vector stmts, String next, Integer threadIndex) throws DistPcalTranslateException {
        /**************************************************************
        * The arguments are:                                               *
        *                                                                  *
...
//        else  result1.addElement(UpdatePC(next));
    ...
    private static Vector 
          CopyAndExplodeLastStmtWithGoto(Vector stmts, String next, Integer threadIndex) throws DistPcalTranslateException {
      /*********************************************************************
    ...
    private static Vector ExplodeLabeledStmt (AST.LabeledStmt ast,
                                              String next, Integer threadIndex) throws DistPcalTranslateException {
         /******************************************************************
         * label SL -->                                                    *
         * label when pc = "label" ;                                       *
         *       SL                                                        *
    ...
    private static Vector ExplodeWhile(AST.LabeledStmt ast,
        ...
        *******************************************************************/
         ...
         * of the While. 
         */
        DistPCalLocation newastBeginLoc = ast.getOrigin().getBegin() ;
        DistPCalLocation whileBeginLoc = w.getOrigin().getBegin() ;
        DistPCalLocation whileBeginUnlabDoLoc = 
          (w.unlabDo.size() != 0) ? 
           ((AST) w.unlabDo.elementAt(0)).getOrigin().getBegin() :
            w.test.getOrigin().getEnd();
        DistPCalLocation whileEndUnlabDoLoc =
          (w.unlabDo.size() != 0) ? 
            ((AST) w.unlabDo.elementAt(w.unlabDo.size()-1)).getOrigin().getEnd() :
            w.test.getOrigin().getEnd();
        DistPCalLocation whileEndLoc = 
    ...
    private static Vector ExplodeLabelIf(AST.LabelIf ast, String next, Integer threadIndex) throws DistPcalTranslateException {
        /***************************************************************
    ...
    * newly created statements for error reporting.                        
     **
    ***********************************************************************/
    private static Vector ExplodeCall(AST.Call ast, String next, Integer threadIndex) throws DistPcalTranslateException {
        Vector result = new Vector();
        ...
        *******************************************************************/
        if (to == st.procs.size())
          { throw new DistPcalTranslateException("Call of non-existent procedure " + ast.to,
                                    ast);  } ;
        DistPcalSymTab.ProcedureEntry pe =
            (DistPcalSymTab.ProcedureEntry) st.procs.elementAt(to);
        /*******************************************************************
        ...
        *******************************************************************/
        if (pe.params.size() != ast.args.size())
            throw new DistPcalTranslateException(
                  "Procedure " + ast.to +
    ...
    * newly created statements for error reporting.                        
     **
    ***********************************************************************/
    private static Vector ExplodeReturn(AST.Return ast, String next) throws DistPcalTranslateException {
        Vector result = new Vector();
        /*******************************************************************
        ...
        * comments for ParseDistAlgorithm.GetReturn.                           *
        *                                                                  *
        ...
        *******************************************************************/
        ...
        if (ast.from == null)
        {
            throw new DistPcalTranslateException("`return' statement not in procedure", ast);
        }
        ;
        ...
        if (!(from < st.procs.size())) {
        	throw new DistPcalTranslateException("Error in procedure (perhaps name used elsewhere)", ast);
        }
        DistPcalSymTab.ProcedureEntry pe =
            (DistPcalSymTab.ProcedureEntry) st.procs.elementAt(from);
        /*********************************************************
    ...
    * newly created statements for error reporting.                        
     **
    ***********************************************************************/
    private static Vector ExplodeCallReturn(AST.CallReturn ast, String next, Integer threadIndex) throws DistPcalTranslateException {
        Vector result = new Vector();
        /*******************************************************************
        ...
        *******************************************************************/
        ...
        int from = st.FindProc(ast.from);
        DistPcalSymTab.ProcedureEntry peFrom =
            (DistPcalSymTab.ProcedureEntry) st.procs.elementAt(from);
        int to = st.FindProc(ast.to);
        /*******************************************************************
        ...
        *******************************************************************/
        if (to == st.procs.size())
          { throw new DistPcalTranslateException("Call of non-existent procedure " + ast.to,
                                    ast);  } ;
        DistPcalSymTab.ProcedureEntry peTo =
            (DistPcalSymTab.ProcedureEntry) st.procs.elementAt(to);
        DistPcalDebug.Assert(from < st.procs.size());
        AST.Assign ass = new AST.Assign();
        ass.ass = new Vector();
        ass.line = ast.line ;
        ass.col  = ast.col ;
    ...
    * goto.                                                                *
    ***********************************************************************/
    private static Vector ExplodeCallGoto(AST.CallGoto ast, String next, Integer threadIndex) throws DistPcalTranslateException {
    ...
    private static AST.Thread ExplodeThread(distpcal.AST.Thread ast, Integer threadIndex) throws DistPcalTranslateException {
        /*********************************************************************
        * Generate new AST.Process with exploded labeled statements.         *
        *********************************************************************/
        int i = 0;
        distpcal.AST.Thread newast = new distpcal.AST.Thread();
        newast.setOrigin(ast.getOrigin()) ;
        newast.col = ast.col;
        newast.line = ast.line;
        newast.name = ast.name;
...
//    private static Vector ExplodeSendAndReceiveCall(AST ast, String next) throws DistPcalTranslateException {
//        Vector result = new Vector();
//        
//        AST.Assign ass = new AST.Assign();
//        ass.ass = new Vector();
