            ...
            for (int j = 0; j < expLine.size(); j ++) {
              ...
              for (int k = 0 ; k < tok.getBeginSubst().size(); k++) {
                  mapLine.addElement(new MappingObject.LeftParen(
                                       (DistPCalLocation) tok.getBeginSubst().elementAt(k)));
              }
              if (tok.source == null) {
              ...
              for (int k = tok.getEndSubst().size()-1 ; k >= 0; k--) {
                  mapLine.addElement(new MappingObject.RightParen(
                         (DistPCalLocation) tok.getEndSubst().elementAt(k)));
              }
            }
...
//          { DistPcalDebug.ReportBug("insertNewToken called with lineNum too big");}
//        Vector curLine = (Vector) tokens.elementAt(lineNum) ;
//
//        if (tokNum >= curLine.size())
//          { DistPcalDebug.ReportBug("insertNewToken called with tokNum too big"); }
//    
...
//System.out.println("Return");
  ...
  /***************** private and debugging methods *********************/
    ...
    ***********************************************************************/
    { int i = 0 ;
      String result = "" ;
      if (i < 0) {DistPcalDebug.ReportBug(
    ...
    ************************************************************************/
    public void print(String name) 
      { DistPcalDebug.print2DVector(tokens, name + ".tokens") ;
        DistPcalDebug.printObjectArray(anchorTokens, name + ".anchorTokens") ;
        DistPcalDebug.printIntArray(anchorTokCol, name + ".anchorTokCol") ;
      }
