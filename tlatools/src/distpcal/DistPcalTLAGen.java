...
/****************************************************************************
 * Given an exploded and disambiguated AST, generate the equivalent TLA+. <br>
 * {@link DistPcalTLAGen#generate(AST, DistPcalSymTab)} returns a vector of
 * Strings, one entry per line of generated TLA+.
 * 
 * @version $Id$
 * @author Leslie Lamport (modified on Thu 6 March 2008 at 10:16:22 PST) (minor
 *         change on 9 December 2009)
 * @author keith (modified on Mon 3 Oct 2005 at 21:43:09 UT)
 * 
 ****************************************************************************/
public class DistPcalTLAGen {
	...
	// be set by options. They are initialized in DistPcalParams.resetParams().
	...
	/**
	 * The tlacode field accumulates the translation as it is constructed. It should
	 * be a vector of separate lines. Keiths original implementation put multiple
	 * lines in a single element of tlacode in:
	 * 
	 * GenVarsAndDefs GenVarDecl
	 */
	private Vector tlacode = new Vector(); /* of lines */

	/**
	 * The tlacodeNextLine field accumulates characters for the next line of
	 * tlacode. It is always a string. It is assumed that when it equals "", then a
	 * new line can be started without adding the current string in tlacodeNextLine
	 * as a new line.
	 */
	private String tlacodeNextLine = "";

	/**
	 * mappingVector is a local pointer to
	 * {@link TLAtoDistPCalMapping#mappingVector}, which is used to accumulate the
	 * TLA+ to PlusCal mapping. It approximately reflects the TLA+ that has been
	 * inserted in the {@link DistPcalTLAGen#tlacode} vector. It is set in the
	 * {@link TLAtoDistPCalMapping#generate} method.
	 */
	private Vector mappingVector;

	/**
	 * mappingVectorNextLine contains the sequence of MappingObject objects that
	 * correspond to the strings added to tlacodeNextLine.
	 */
	private Vector mappingVectorNextLine = new Vector();

	/**
	 * The self field is set to "self" by GenProcess when called for a single
	 * process (rather than a process set) and by GenProcedure for a multiprocess
	 * algorithm. It is set to the process id by GenProcess when called for a single
	 * process. selfIsSelf is set to true when self is set to "self", and to false
	 * when self is set to a process id. The self field never seems to be reset to
	 * null.
	 */
	private TLAExpr self = null; // changed by LL on 22 jan 2011 from: private String self = null; /* for current
									// process */
	...
	private Vector pcV = new Vector(); /*
										 * sublist of vars of variables representing procedure parameters and procedure
										 * variables
										 */
	private Vector psV = new Vector(); /* sublist of vars local to a process set */
	private DistPcalSymTab st = null; /* symbol table */
	private boolean mp = false; /* true if multiprocess, else unip */
	private Vector nextStep = new Vector(); /* unparam actions */ // For multiprocess alg, these are the individual (=)
																	// processes
	private Vector nextStepSelf = new Vector(); /* param actions */ // These are process sets (\in processes) and
																	// procedures
	 ...
	 * @throws DistPcalTLAGenException on any unrecoverable methods
	 */

	/*
	 * The string currentProcName is the name of the current process (for a
	 * multiprocess algorithm) or "Next" for a uniprocess algorithm. When
	 * ParseDistAlgorithm.omitPC is true, this is used instead of the label when
	 * generating the process's or the entire algorithm's next-state action. Thus,
	 * with a single label lbl, this generates Next == Xlation instead of lbl ==
	 * Xlation Next == lbl
	 */
	 ...
	 * @throws DistPcalTLAGenException
	 */
	public Vector<String> generate(AST ast, DistPcalSymTab symtab, Vector report) throws DistPcalTLAGenException {
		 ...
		 * translation algorithm if the user selects the entire algorithm, in which case
		 * it will return the null region to GotoPCalSourceHandler.execute.
		 */
		DistPCalLocation ZeroLocation = new DistPCalLocation(0, 0);
		...
		/* ------------------ end testing -------------------------- */
		Vector nonredundantMappingVector = TLAtoDistPCalMapping.RemoveRedundantParens(mappingVector);
	...
	/**
	 ****************************************************************/
	private void GenSym(AST ast, String context) throws DistPcalTLAGenException {
		if (ast.getClass().equals(AST.UniprocessObj.getClass()))
			...
			GenMultiNodes((AST.MultiNodes) ast, context);
		...
		else if (ast.getClass().equals(AST.NodeObj.getClass()))
			GenNodes((AST.Node) ast, context);
		else if (ast.getClass().equals(AST.LabeledStmtObj.getClass()))
			GenLabeledStmt((AST.LabeledStmt) ast, context);
	}

	private void GenUniprocess(AST.Uniprocess ast, String context) throws DistPcalTLAGenException {
		mp = false;
	  ...
	  private void GenMultiprocess(AST.Multiprocess ast, String context) throws DistPcalTLAGenException
	  {
		  mp = true;
	...
	private void GenMultiNodes(MultiNodes ast, String context) throws DistPcalTLAGenException {
		mp = true;
	...
	private void GenProcedure(AST.Procedure ast, String context) throws DistPcalTLAGenException {
		/*
		 * First, generate the body's actions. Must set self and selfIsSelf (?) for use
		 * by GenLabeledStmt.
		 */
		if (mp) {
...
//            /* Add this step to the disjunct of steps with (self) */
		...
		for (int i = 0; i < ast.body.size(); i++) {
			...
			if (i != 0 && tlacodeNextLine.length() + 7 /* the 7 was obtained empirically */
...
//        tlacode.addElement("");
	 ...
	 private void GenProcess(AST.Process ast, String context) throws DistPcalTLAGenException
	 {
	...
	private void GenNodes(Node node, String context) throws DistPcalTLAGenException {
		...
		for(distpcal.AST.Thread thread : node.threads) {
			for (int j = 0; j < thread.body.size(); j++) {
				AST.LabeledStmt tStmt = (AST.LabeledStmt) thread.body.elementAt(j);
				//special handling for while especially and initial while
...
//        tlacode.addElement("");
	...
	/**
	 ****************************************************/
	private void GenLabeledStmt(AST.LabeledStmt ast, String context) throws DistPcalTLAGenException {
		...
		// testing and setting of pc ensures that there is more than one element in
		// ast.stmts.
		 ...
		 * output, but it seems safer to maintain the current structure in which each
		 * GenX for each statement type X does that.
		 */

		/*
		 * We set macroBeginLeft to the macroOriginBegin field of the first statement in
		 * ast.stmts with a non-null origin, and macroEndRight to the macroOriginEnd
		 * field of the last statement in ast.stmts with a non-null origin.
		 */
		DistPCalLocation macroBeginLeft = null;
		DistPCalLocation macroEndRight = null;
		 ...
		 * tlacode.addElement call with a call of addOneLineOfTLA--except that the last
		 * one is replaced with a call of addOneTokenToTLA so we can put the RightParen
		 * object in mappingVector.
		 */
...
//        				+ c.Unchanged());
				addOneTokenToTLA(NSpaces(col) + "/\\ " + c.Unchanged() + "' = " + c.Unchanged());
			}
		} else {
			...
			if (ast.stmts.size() == 1) {
				for (int i = defStartLine; i < tlacode.size(); i++) {
					...
					if (i == defStartLine) {
						// remove the "/\ " added
						tlacode.setElementAt(
								line.substring(0, colAfterAnd - 3) + line.substring(colAfterAnd, line.length()), i);
		...
		/*
		 * We call addRightParenV rather than addRightParen because it the last
		 * statement that came from the PCal code arose from the expansion of a macro
		 * call, then we want the RightParen's location to be the end of the call, not
		 * the end of the macro's code.
		 */
...
//        tlacode.addElement("");
	...
	/**
	 * Adjusts the objects in line lineNum of mappingVector so all column numbers
	 * starting with startCol are decreased by `shift'. If any Begin/EndTLAToken
	 * pairs are changed to have a non-positive width, a bug is reported.
	 * 
	 * Note: It is assumed that there is no aliasing of MappingTokens in
	 * mappingVector. That is, other than in transient local variables, the only
	 * pointer to a MappingToken in mappingVector is the single one in its line of
	 * mappingVector. I can't see how any aliasing of MappingTokens could arise.
	 * 
			...
			if (obj.getType() == MappingObject.BEGIN_TLATOKEN) {
				...
				if (obj.getType() == MappingObject.END_TLATOKEN) {
					...
					if (lastWasBeginTLAToken && tobj.getColumn() <= lastBeginTLATokCol) {
						DistPcalDebug.ReportBug("PcalTLAGen.shiftMappingVectorTokensLeft created a null TLA Token");
					}
	...
	/***************************************************************************
	 * LL Comment added 27 Jan 2006: * * There is a basic flaw in the way GenStmt
	 * works. It now generates the * output on the fly. This means that * * - There
	 * is no way to avoid the prefix /\ on a one-element conjunct * because GenStmt
	 * has no way of knowing if there's another conjunct * coming. * * - The
	 * handling of the UNCHANGEDs of the THEN and ELSE clauses of * an IF is a
	 * kludge, because the UNCHANGED of the THEN clause is * output before it can be
	 * known. * * The correct way of doing things is to define GenStmt so it returns
	 * a * sequence (vector) of string vectors, each string vector being a *
	 * conjunction expression (without a leading /\ or any leading spaces) and * the
	 * new Changed object (which it can do as it now does by modifying its * Changed
	 * object argument). It would also be useful to define a * GenStmtSeq that
	 * simply calls GenStmt iteratively on a sequence of * simple statements. The
	 * method that calls GenStmtSeq would then add the * Unchanged conjunct and call
	 * a method that returns a sequence of * conjuncts and a prefix into a string
	 * vector containing the prefix and * the necessary /\s. *
	 ***************************************************************************/
	...
	/**
	 ****************************************************************/
	private void GenStmt(AST ast, Changed c, String context, String prefix, int col) throws DistPcalTLAGenException {
...
//        Vector vlines = new Vector();
		/*
		 * Sort the vector ast.ass so that assignments to the same variable follow one
		 * another.
		 */
		...
		int numAssigns = 0;
		/*
		 * hasMultipleVars set true iff the assignment assigns values to more than one
		 * variable, and hence the statement's translation has multiple conjuncts.
		 */
		...
		while (i < ast.ass.size()) {
			...
			/*
			 * Added by LL and MK on 16 May 2018: Report an error if the variable being
			 * assigned is not a variable declared in the algorithm (either not declared at
			 * all or is a constant, bound identifier, ...).
			 */

			if (!this.vars.contains(sF.lhs.var)) {
				throw new DistPcalTLAGenException("Assignment to undeclared variable " + sF.lhs.var, sF);
			}
			...
			/*
			 * Throws an error if there are multiple assignments to the variable in
			 * different statements, or if there are multiple assignments to the variable in
			 * this statement and at least one of them has no subscript.
			 */
			if (cThis.Set(sF.lhs.var) > 1 || (iLast - iFirst > 0 && hasAssignmentWithNoSubscript)) {
				/***********************************************************
				 * The following was changed by LL on 3 Mar 06 to use * AST.location to properly
				 * report the location of an * error in a line created by expanding a macro. *
				 * However, it doesn't work very well otherwise. This * should be fixed. *
				 ***********************************************************/
				throw new DistPcalTLAGenException("Multiple assignment to " + sF.lhs.var, ast /* sF */);
			}
...
//                        lines.addElement(sb.toString());
					...
					/*****************************************************
					 * Was * * Vector sv = sass.lhs.sub.toStringVector(); * * Changed by Chi Ho on 3
					 * Aug 2006 to add * subscript. See bug_06_08_03. *
					 *****************************************************/
					if (sv.size() > 0) {
...
//System.out.println("origin: " + rhs.getOrigin().toString() );
				...
				/*
				 * Multiple assignments to the same variable, which must therefore each have a
				...
				int cc = sb.length();
				/*
				 * If this the first variable, so i = 0, then sb does not contain any spaces to
				 * compensate for the missing prefix; otherwise it does.
				 */
				if (i == 0) {
					cc = cc + prefix.length();
				}
				boolean subscript = (mp && (IsProcedureVar(sass.lhs.var) || IsNodeSetVar(sass.lhs.var)));
				while (iFirst <= iLast) {
...
//        }
	...
	/**
	 * Generate TLA+ for if statement. Each branch has its own UNCHANGED that lists
	 * variables that were changed in the other branch. This is a little difficult
	 * since we don't know the UNCHANGED for the Then branch until the code for the
	 * Else branch is generated. So, we fix the line in the Then branch after the
	 * Else branch is done. The corresponding mappingVector line also has to be
	 * changed, but that's not a problem because the UNCHANGED is a TLA+ token with
	 * no corresponding source.
	 */
	private void GenIf(AST.If ast, Changed c, String context, String prefix, int col) throws DistPcalTLAGenException {
...
//        tlacode.addElement(sb.toString());
		...
		for (int i = 0; i < ast.Then.size(); i++) {
			GenStmt((AST) ast.Then.elementAt(i), cThen, context, sb.toString(),
					/*******************************************************************
					 * LL added the +3 on 18 Feb 2006 to take account of the * indentation of the
					 * "IF ". *
					 *******************************************************************/
					here + 3);
			sb = new StringBuffer(NSpaces(here) + "/\\ ");
...
//            tlacode.addElement(sb.toString());
			...
			endCurrentLineOfTLA();
														...
														// unchanged variable v, it produces v' = v if v is a short
														// variable,
														// otherwise it produces UNCHANGED v
														//
...
//            tlacode.addElement(sb.toString());
			...
			/*
			 * There is no UNCHANGED after the ELSE, so we have to put the RightParen for
			 * the whole if statement at the end of the last line already generated
			 */
	...
	/***********************************************************************
	 * Added by LL on 30 Jan 2006. * * Generate TLA+ for the `either' statement.
	 * This performs the same * sort of hackery as for the `if' statement,
	 * necessitated by the * design flaw commented on above.
	 **
	 ***********************************************************************/
	private void GenEither(AST.Either ast, Changed c, String context, String prefix, int col)
		...
		/******************************************************************
		 * tlaout.elementAt(ucLocs[i]) is the UNCHANGED clause for the * i-th `or'
		 * clause. *
		 ******************************************************************/
		StringBuffer sb = new StringBuffer(prefix);
		...
		int here = sb.length();
		/*******************************************************************
		 * The number of columns to the left of the code generated for * each `or'
		 * clause. *
		 *******************************************************************/
		...
		addLeftParen(ast.getOrigin());
		/*********************************************************************
		 * Produce the output for the clauses, but with a dummy line in * place of the
		 * UNCHANGED clause, and compute allC, cOrs, and * ucLocs. *
		 *********************************************************************/
		for (int i = 0; i < ast.ors.size(); i++) {
			...
			for (int j = 0; j < orClause.size(); j++) {
				/***********************************************************
				 * On 6 Jun 2010, LL added the "+3" in the following call * of GenStmt. This
				 * seems to fix a bug which caused * * either when \/ A * \/ B * or ... * * to
				 * produce * \/ /\ \/ A * \/ B * \/ ... *
				 ***********************************************************/
...
//            tlacode.addElement("Replace by UNCHANGED"); // 
		...
		/**********************************************************************
		 * Insert real UNCHANGED clauses. Note that we have to go through * loop
		 * backwards since we will remove a line of output for each `or' * clause that
		 * doesn't get an UNCHANGED. *
		 **********************************************************************/
		int i = ast.ors.size();
		while (i > 0) {
			i = i - 1;
			...
			if (numUnchanged > 1) {
				...
				mappingVector.insertElementAt(stringToTLATokens(line), ucLocs[i]);
											...
											// otherwise it produces UNCHANGED v
											//
											// tlacode.insertElementAt(NSpaces(here) + "/\\ UNCHANGED " + NotChanged,
											// ucLocs[i]);
...
//            				+ NotChanged, ucLocs[i]);
	...
	private void GenWith(AST.With ast, Changed c, String context, String prefix, int col)
			throws DistPcalTLAGenException {
		addLeftParen(ast.getOrigin());
		StringBuffer sb = new StringBuffer(prefix);
...
//            tlacode.addElement(sb.toString());
			...
			/*************************************************************
			 * LL changed "col + 4" to "col + 2" here to correct an * alignment problem on
			 * 31 Jan 2006. *
			 *************************************************************/
			if (ast.Do.size() > 1)
				sb.append("/\\ ");
...
//            tlacode.addElement(sb.toString());
	...
	private void GenWhen(AST.When ast, Changed c, String context, String prefix, int col)
			throws DistPcalTLAGenException {
		addOneTokenToTLA(prefix);
...
//        tlacode.addElement(prefix + "TRUE");
	...
	/***********************************************************************
	 * Generate the VARIABLES declaration(s), output the TLA+ "code" from * a
	 * `define' statement, if any, and generate the definition of * `vars'. * *
	 * Method renamed from GenVars and given the defs argument by LL on * 25 Jan
	 * 2006 to handle the `define' statement. *
	 ***********************************************************************/
	private void GenVarsAndDefs(Vector globals, Vector procs, Vector processes, TLAExpr defs, Vector nodes)
			throws DistPcalTLAGenException {
		/*******************************************************************
		 * lVars and gVars are vectors of strings, each element being a * variable name.
		 * They hold the local and global variables, * respectively. *
		 *******************************************************************/
		Vector lVars = new Vector();
		Vector gVars = new Vector();

		/**
		 * lVarsSource and gVarsSource are vectors of AST.VarDecl objects that generated
		 * the elements of lVars and gVars, where PVarDecl objects are converted to
		 * VarDecl objects.
		 */
		...
		/*******************************************************************
		 * Set gVars to the global variables, including pc and `stack' if * there are
		 * procedures, and add these variables to vars. *
		 *******************************************************************/
		if (globals != null)
		...
		/*******************************************************************
		 * Add local process variables to lVars, vars, and psV for * variables local to
		 * process sets. *
		 *******************************************************************/
		if (processes != null)
		...
		/*******************************************************************
		 * Add local node variables to lVars, vars, and psV for * variables local to
		 * node sets. *
		 *******************************************************************/
		if (nodes != null)
		...
		if (EmptyExpr(defs)) {
			/******************************************************************
			 * There is no `define' statement. In this case, generate a * single VARIABLES
			 * statement and set gVars to vector of all * variables. *
			 ******************************************************************/
			gVars.addAll(lVars);
			...
			/******************************************************************
			 * There is a `define' statement. In this case, must declare * global and local
			 * variables separately. Also, set gVars to * vector of all variables. *
			 ******************************************************************/
			GenVarDecl(gVars, gVarsSource);
			addOneLineOfTLA("");
...
//            }
		...
		/*
		 * We check for the unlikely case in which there are no variables. Without this
		 * check, the Init is not generated but appears in the definition of Spec.
		 */
		if (gVars.size() == 0) {
			throw new DistPcalTLAGenException("The algorithm has no variables.");
		}
		/*******************************************************************
...
//        tlacode.addElement(var.toString());
	 ...
	 * strings that are the variables to be declared. It does nothing if the vector
	 * has length 0. The varVecSource argument is a vector of the same size as
	 * varVec that contains the AST.VarDecl objects.
	 * <p>
	 * Method added by LL on 25 Jan 2006.
	 * 
	 * Modified 16 Dec 2011 to add varVecSource argument and generate TLA to PCal
	 * mapping.
	 * 
...
//        tlacode.addElement(res.toString());
	...
	/**
	 **********************************/
	private void GenInit(Vector globals, Vector procs, Vector processes, Vector nodes) throws DistPcalTLAGenException {
		int col = "Init == ".length();
			...
			for (int i = 0; i < procs.size(); i++) {
				...
				for (int p = 0; p < proc.params.size(); p++) {
					...
					if (!mp) {
						 ...
						 * expression if needed. Also replaced * test for "\\in" with assertion that it
						 * can't occur, * since it's forbidden by the grammar. *
						 *******************************************************/
						...
						addExprToTLA(
								AddSubscriptsToExpr(decl.val, SubExpr(Self("procedure")), new Changed(new Vector())));
				...
				for (int p = 0; p < proc.decls.size(); p++) {
					 ...
					 * procedure variables. (Well done, Keith!) I realized this too late to feel
					 * like procedurizing it.
					 */
					AST.PVarDecl decl = (AST.PVarDecl) proc.decls.elementAt(p);
					if (!mp) {
						addVarDeclToTLA(decl.toVarDecl(), is);
...
//                    is.append(decl.var);
		...
		if (processes != null && processes.size() > 0) {
			...
			for (int i = 0; i < processes.size(); i++) {
				...
				for (int p = 0; p < proc.decls.size(); p++) {
					/*
					 * In the comments below, (( and )) represent MappingObject.LeftParen and
					 * MappingObject.RightParen objects.
					 */
					AST.VarDecl decl = (AST.VarDecl) proc.decls.elementAt(p);
					is.append("/\\ ");
					/*
					...
					if (proc.isEq) {
						...
						if (decl.isEq) {
							 ...
							 * /\ (( v = [self \in (( S )) |-> (( ValBar )) ] ))
							 * 
							 * where ValBar obtained from Val by replacing each variable w of the process
							 * with w[self].
							 */
							is = new StringBuffer(decl.var);
							is.append(" = [self \\in ");
							...
							addLeftParen(decl.val.getOrigin());
							addExprToTLA(AddSubscriptsToExpr(decl.val, SubExpr(Self("procedure")),
							 ...
							 * /\ (( v \in [ (( S )) -> (( ValBar )) ] ))
							 * 
							 * where ValBar is obtained from Val by replacing each variable w of the process
							 * with
							 * 
							 * w[CHOOSE self \in S : TRUE]
							 * 
							 * We first set expr to the TLAExpr "CHOOSE self \in S : TRUE". (This is Keith's
							 * original code.)
							 */
							...
							try {
								...
								throw new DistPcalTLAGenException(e.getMessage());
							}
							...
							addOneTokenToTLA(" -> ");
							addLeftParen(decl.val.getOrigin());
							addExprToTLA(AddSubscriptsToExpr(decl.val, expr, new Changed(new Vector())));
							addRightParen(decl.val.getOrigin());
							addOneTokenToTLA("]");
						}
					}
		...
		if (nodes != null && nodes.size() > 0) {
			...
			for (int i = 0; i < nodes.size(); i++) {
				AST.Node node = (AST.Node) nodes.elementAt(i);
				if (node.decls.size() == 0) // No variables in this procedure
					continue;
				is.append("(* Node ");
				is.append(node.name);
				is.append(" *)");
				...
				for (int n = 0; n < node.decls.size(); n++) {
					...
					if (node.isEq) {
						...
						if (decl.isEq) {
							...
							TLAExpr subexpr = node.id.cloneAndNormalize();
							TLAExpr expr = new TLAExpr();
							expr.addLine();
							expr.addToken(new TLAToken("[", 0, TLAToken.BUILTIN));
							expr.addToken(new TLAToken("CHOOSE", 1, TLAToken.BUILTIN));
...
//            tlacode.addElement(is.toString());
		...
		if (!ParseDistAlgorithm.omitPC) {
			if (mp) {
				...
				if (boxUnderCASE)
					colPC = colPC - 3;
				for (int n = 0; n < st.nodes.size(); n++) {
					DistPcalSymTab.NodeEntry ne = (DistPcalSymTab.NodeEntry) st.nodes.elementAt(n);
					if (useCase) {
						is.append("self ");
						if (ne.isEq) {
							is.append("= ");
					...
					if (boxUnderCASE && n < st.nodes.size() - 1)
						is.append("[] ");
				}
			} else {
				//uninode case ==> not really tested
				is.append("/\\ pc = \"" + st.iPC + "\"");
				addOneLineOfTLA(is.toString());
			}
		}
...
//        ps.append("ProcSet == ");
        ...
        {
            DistPcalSymTab.ProcessEntry proc = (DistPcalSymTab.ProcessEntry) st.processes.elementAt(i);
...
//        tlacode.addElement("");
	...
	private void GenNext() {
		...
		// already been added.
		if (ParseDistAlgorithm.omitPC && !mp) {
			return;
		}
		...
		if (!(DistPcalParams.NoDoneDisjunct || ParseDistAlgorithm.omitStutteringWhenDone)) {
//          tlacode.addElement(sb.toString());
			...
			if (mp) {
				 ...
				 ************************************************************/
				/************************************************************
				 * Changed by MK on 19 Jun 2019 into a conjunct list * after modifying GenNext
				 * to generate an explicit * Terminating action before Next instead of the old *
				 * implicit disjunct-to-prevent-deadlock-on-termination. * This change also
				 * entailed to copy this if-block from the * end of GenNext here modifying the
				 * original one to * generate a call to Terminating. * * The rational for this
				 * change results from the recently * introduced TLC profiler. The profiler
				 * reports the number * of distinct successor states per action. * A terminating
				 * PlusCal algorithm has the implicit *
				 * disjunct-to-prevent-deadlock-on-termination sub-action of * the Next
				 * next-state action. Since the sub-action has no * identifier, the profiler has
				 * to report Next as generating * no successor states (which is bogus). With
				 * this change, * the profiler will report the Terminating sub-action to *
...
//              tlacode.addElement(sb.toString());
		...
		if (mp && st.nodes.size() > 0)
			for (int i = 0; i < st.nodes.size(); i++) {
				DistPcalSymTab.NodeEntry n = (DistPcalSymTab.NodeEntry) st.nodes.elementAt(i);
				if (n.isEq)
					continue;
	...
	/****************************************/
	/***********************************************************************
	 * The spec can contain the following conjuncts * * 1. Init /\ [][Next]_vars *
	 * Always present * * 2. WF_var(Next) * present if (a) The wfNext option is
	 * specified, or * (b) It is a uniprocess algorithm and one of the * options
	 * -wf, -sf, -termination is specified. * * 3. A sequence of process fairness
	 * formulas, containing * one for each process for which there is a fairness
	 * condition. * * A process has * (a) a WF fairness condition iff * (i) it is
	 * preceded by the keyword "fair" and the -nof * option is not specified, or *
	 * (ii) the -wf option is specified. * (b) an SF fairness condition iff it is
	 * not preceded * by the keyword "fair" and the -sf option is specified. * * Let
	 * P be a process specified by either * * [fair] process (P = exp) ... * [fair]
	 * process (P \in exp) ... * * In the first case we say that P is a single
	 * process, in the second * that it is a process set. Let * * - p_1, ... , p_Np
	 * be the labels of P modified by "+" * * - m_1, ... , m_Nm be the set of labels
	 * of P modified by "-" * * - pSelf = IF P is a single process THEN "pname" *
	 * ELSE "self" * * - qSelf = IF /\ P is a single process * /\ pSelf a multi-line
	 * formula * THEN "self" * ELSE pSelf * * - pName = IF P is a single process
	 * THEN "P" * ELSE "P(self)" * * A process fairness formula is described by the
	 * following: * * XF: * either WF or SF depending on P's fairness condition * *
	 * prefix: * IF P is a single process * THEN IF pSelf a multi-line formula *
	 * THEN "LET self = pSelf IN" * ELSE "" * ELSE "\A self \in exp" * * wfFormula:
	 * * "XF( (pc[qSelf] \notin {"m_1", ... , "m_Np"}) /\ pName )" * * sfFormula: *
	 * if XF = SF * then null * else if P a single process * then "SF_vars(p_1) /\
	 * ... /\ SF_vars(p_Np)" * else "SF_vars(p_1(self)) /\ ... * /\
	 * SF_vars(p_Np(self))" * * prcdFormulas: * A sequence consisting of the
	 * following two formulas for each * procedure D called within P. Let * * -
	 * pd_1, ... , pd_Npd be the labels of D modified by "+" * - md_1, ... , md_Nmd
	 * be the labels of D modified by "-" * * in the formulas: * * wfFormula: * "XF(
	 * (pc[qSelf] \notin {"md_1", ... , "md_Nmd"}) * /\ D(qSelf) )" * * sfFormula: *
	 * if XF = SF * then null * else "SF_vars(pd_1(qSelf)) /\ ... * /\
	 * SF_vars(pd_Npd(qSelf))" * * ------- * * If there is at least one fairness
	 * formula, the definition of Spec * will be formatted in one of two ways. If
	 * there is either a * WF_vars(Next) condition or a process fairness formula,
	 * then it may * be formatted as: * * Spec == Init /\ [][Next]_vars * *
	 * otherwise, it will be formatted as * * Spec == /\ Init /\ [][Next]_vars * [/\
	 * WF_vars(Next)] * /\ F_1 * ... * /\ F_n * * where each F_i is a process
	 * fairness formulas. *
	 ***********************************************************************/
	private void GenSpec() {
		String safetyFormula = "Init /\\ [][Next]_vars";

		if (DistPcalParams.FairnessOption.equals("nof") || (!mp && DistPcalParams.FairnessOption.equals(""))) {
		...
		if (DistPcalParams.FairnessOption.equals("wfNext") || DistPcalParams.FairAlgorithm || (!mp
				&& (DistPcalParams.FairnessOption.equals("wf") || DistPcalParams.FairnessOption.equals("sf")))) {
		...
		if (mp) {
			for (int i = 0; i < st.nodes.size(); i++) {
				...
				if (fairness != AST.UNFAIR_PROC) {
					String xf = (fairness == AST.WF_NODE) ? "WF" : "SF";
					...
					for (int k = 0; k < procedures.size(); k++) {
						...
						String name = st.UseThis(DistPcalSymTab.PROCEDURE, originalName, "");
						int procedureIndex = st.FindProc(name);
						DistPcalSymTab.ProcedureEntry pe = (DistPcalSymTab.ProcedureEntry) st.procs
					...
					procFairnessFormulas.addElement(new NodeFairness(xf, prefix, wfSB.toString(),
		...
		for (int i = 0; i < procFairnessFormulas.size(); i++) {
			/*
			 * The original code called format on the fairness formula, which can create a
			 * string with \n characters embedded. I've just split the string into its
			 * individual lines and added them to tlacode one at a time. However, the
			 * current and original code can both produce very long lines that could be
			 * wrapped. So if this change does make a difference, then it would be better to
			 * completely rewrite the format method (which is only called here).
			 */
			String str = "        /\\ " + ((NodeFairness) procFairnessFormulas.elementAt(i)).format(indent).toString();
...
//        tlacode.addElement("");
	...
	/* be primed. */
	/**********************************************************/
	private TLAExpr AddSubscriptsToExpr(TLAExpr exprn, TLAExpr sub, Changed c) throws DistPcalTLAGenException {
		/*
...
//            depths[i] = parenDepth;
		...
		/*
		 * We now set stringVec to the sequence of identifiers that occur in exprn for
		 * which we need to add a subscript or a prime.
		 */
		...
		for (int i = 0; i < expr.tokens.size(); i++) {
			...
			for (int j = 0; j < tv.size(); j++) {
				...
				boolean subr = (sub != null && (tok.type == TLAToken.ADDED || (mp && (tok.type == TLAToken.IDENT)
						&& (IsProcedureVar(tok.string) || IsNodeSetVar(tok.string)))));
				if ((subr || prime) && !InVector(tok.string, stringVec)) {
					 ...
					 * 
					 * that is commented out. Note that this change can add a token with type ADDED
					 * rather than IDENT. I don't think this matters.
					 */
					TLAToken newTok = tok.Clone();
					/*
					 * The new token should inherit nothing from the baggage of tok, whose only
					 * function is to provide the name
					 */
					...
					if (prime) {
						/*****************************************************
						 * Modified by LL on 30 Aug 2007. The following * call to addTokenOffset was
						 * originally a call to * addToken. See the comments for *
					...
					/**********************************************************
					 * Modified by LL on 31 Jan 2006 to comment out the call * of MakeExprPretty,
					 * since it totally screwed up the * formatting when substituting any string
					 * containing * spaces or multiple lines for a variable. *
					 **********************************************************/
					// MakeExprPretty(exp);
...
//            depths[i] = parenDepth;
	...
	/***********************************************************************
	 * Given an expression, makes it into a subscript expr. It is called * only with
	 * argument Self(context), which means that it is called * only for a single
	 * subscript. * * If string is null, then returns null. * * Since this is used
	 * only to add "[self]", there is no need to add * any REPLACEMENT tokens to the
	 * expression. Moreover, the added * subscript's tokens have a null source field
	 * because they don't come * from any PCal code. The new expression is given the
	 * same origin as * the original one. *
	 ***********************************************************************/
	private static TLAExpr SubExpr(TLAExpr sub) {
		if (sub != null) {
			...
			for (int i = 0; i < expr.tokens.size(); i++) {
				...
				if (i == 0) {
					/*
					 * Set isAppended field of the "[" and "]" to true iff the first token of sub
					 * has isAppended field true, which should be the case iff it is the "self"
					 * token.
					 */
					tokenVec.insertElementAt(new TLAToken("[", 0, TLAToken.BUILTIN, sub.firstToken().isAppended()), 0);
				}
			}
			expr.addTokenOffset(new TLAToken("]", 0, TLAToken.BUILTIN, sub.firstToken().isAppended()), 0);
			return expr;
		} else {
			return null;
		}
	...
	private static TLAExpr selfAsExpr() {
		/*
		 * This is a token that does not correspond to anything in the PCal code, so it
		 * should have a null source field. It has a true isAppended field because it is
		 * always appended as a subscript to a variable.
		 */
...
//        expr.anchorTokCol[0] = 0;
	...
	/***********************************************************************
	 * Comment added by LL: MakeExprPretty should never be called on an * expression
	 * any part of which was an expression in the input. * Fortunately, it is now
	 * called only for the expression "[self]", so * it is effectively a no-op. *
	...
	/***********************************************************************
	 * If vec is a StringVector representing an expression, then this * returns the
	 * StringVector obtained by parenthesizing the expression * if it may need
	 * parenthesizing. This is used only to prevent * parsing errors when the
	 * expression appears immediately to the right * of an "=" in the spec. This is
	 * a rare situation, so it would be * nice to add the parentheses only if really
	 * necessary. For now, the * parentheses are added if one of the following
	 * tokens occur outside * parentheses and not inside a string: * * = * # * < not
	 * followed by < * > not followed by > or preceded by = * | preceded or followed
	 * by - * \ not followed by "o" or "X". * / followed by "\" * * Left parentheses
	 * are * * ( [ { << * * The handling of "\" is a simplifying hack. Lots of
	 * operators * beginning with "\" like "\/", "\gg" and "\subseteq" have
	 * precedence * greater than or equal to "=". The only commonly used ones with *
	 * precedence lower than "=" seem to be "\o" and "\X". It doesn't * seem to be
	 * worth the bother of checking for the others just to * avoid unnecessarily
	 * adding the parentheses when those other rare * operators are used. * *
	 * Perhaps the one improvement that might be worth making in this * procedure is
	 * to have it not add parentheses because of "dangerous" * operations in an IF
	 * clause--for example: * * IF x < 0 THEN ... * * This would require considering
	 * "IF" to be a left parenthesis and * "THEN" to be a right parenthesis.
	 * However, that's not trivial to * implement because of unlikely things like *
	 * * IFx := 42 ; * x := IFx < THENx *
	 ***********************************************************************/
	...
	/**
	 * As part of adding the TLA to PCal translation code, LL removedseparated the
	 * code that decides if parentheses are needed from the Parenthesize method and
	 * put it into this method. The Parenthesize method itself will not be needed.
	 */
	public static boolean NeedsParentheses(Vector vec) {
		...
		;
		/*******************************************************************
		 * vec shouldn't be empty, but let's not worry about what to do if * it is. *
		 *******************************************************************/
		int curCharNum = 0;
		int curLineNum = 0;
		int parenDepth = 0;
		...
		while ((curLineNum < vec.size()) && (!needParen)) {
			...
			while ((curCharNum < curLine.length()) && (!needParen)) {
				...
				if (inString) {
					...
					/***************************************************************
					 * Set nextChar to the next character on the line, or ' ' if * there is none. *
					 ***************************************************************/
					char nextChar = ' ';
	...
	/*
	 * The following methods are used to add code to tlacode and add the appropriate
	 * objects to mappingVector
	 */

	/**
	 * Adds one token to tlacodeNextLine, and adds the appropriate Begin/EndTLAToken
	 * objects to mappingVectorNextLine. The ...TLAToken objects mark a region that
	 * excludes beginning and ending space characters of token--unless the token has
	...
	/**
	 * If region is non-null, then adds string str to the TLA output as a
	 * SourceToken object with that region. Otherwise, it adds it as a TLAToken,
	 * with Begin/EndTLAToken objects.
	 * 
	 * @param str
	 * @param region
	 */
	private void addOneSourceTokenToTLA(String str, Region region) {
		...
		mappingVectorNextLine.addElement(new MappingObject.SourceToken(beginCol, endCol, region));
		tlacodeNextLine = tlacodeNextLine + str;
	}

	/**
	 * Adds a complete line of TLA "code" that does not correspond to any PlusCal
	 * code. Adds Begin/EndTLAToken objects to the mapping iff the line does not
	 * equal "".
	 * 
	 * @param line
	 */
...
// temporarily commented out.
		if (tlacode.size() != mappingVector.size()) {
			DistPcalDebug.ReportBug("tlacode and mappingVector have different lengths");
		}
...
//}
	...
	/**
	 * If tlacodeNextLine does not equal "", then add it to tlacode and add
	 * mappingVectorNextLine to mappingVector. If it does equal "", don't add any
	 * lines, but add any potentially legal leftovers in mappingVectorNextLine to
	 * the previous line.
	 */
	private void endCurrentLineOfTLA() {
		if (tlacodeNextLine.length() != 0) {
			...
			if (mappingVectorNextLine.size() != 0) {
				/*
				 * There's something to go in the mappingVector that doesn't accompany any text.
				 * It should be one or more RightParen or LeftParen objects, (or perhaps,
				 * eventually a Break), in which case they should be put at the end of the
				 * previous mappingVector line. Anything else is a mistake.
				 */
				Vector lastLine = (Vector) mappingVector.elementAt(mappingVector.size() - 1);
				for (int i = 0; i < mappingVectorNextLine.size(); i++) {
					MappingObject obj = (MappingObject) mappingVectorNextLine.elementAt(i);
					if (obj.getType() == MappingObject.RIGHT_PAREN || obj.getType() == MappingObject.LEFT_PAREN
							|| obj.getType() == MappingObject.BREAK) {
						lastLine.add(obj);
					} else {
						DistPcalDebug.ReportBug("PcalTLAGen.endCurrentLineOfTLA found problem.");
					}
					mappingVectorNextLine = new Vector();
				}
			}
	...
	/**
	 * Adds the expression to tlacode / tlacodeNextLine and its mapping to
	 * mappingVector / mappingVectorNextLine. It adds no space before the expression
	 * and leaves the last line of the expression (which could be its first line) at
	 * the end of tlacodeNextLine.
	 * 
	 * @param expr
	 */
	private void addExprToTLA(TLAExpr expr) {
		...
		if (indent != 0) {
			/*
			 * Need to combine first line of expr with tlacodeNextLine.
			 */
	 ...
	 * StringBuffer `is' containing the text that precedes the "/\" of the conjunct,
	 * which will be "Init == " or just spaces.
	 * 
	 * @param decl
	 * @param is
	 */
...
//		expr.anc = decl.val.anchorTokCol;
	...
	/**
	 * Adds a MappingObject.LeftParen object to the mapping vector for the beginning
	 * of the Region region, if it's not null.
	 * 
	 * @param region
	 */
	private void addLeftParen(Region region) {
		if (region != null) {
			mappingVectorNextLine.addElement(new MappingObject.LeftParen(region.getBegin()));
		}
	}
	...
	private void addRightParen(Region region) {
		if (region != null) {
			mappingVectorNextLine.addElement(new MappingObject.RightParen(region.getEnd()));
		}
	}

	/**
	 * Like addLeftParen(ast.getOrigin()), except that it uses loc the location if
	 * it is not null. It is called by GenLabeledStmt and ast.getOrigin() should
	 * never be null. However, if it is, then we don't add a Paren; this insures
	 * that the matching calls of addLeftParenV and addRightParenV both either do or
	 * don't add a Paren.
	 * 
	 * @param ast
	 */
	private void addLeftParenV(AST ast, DistPCalLocation loc) {
		if (ast.getOrigin() == null) {
			return;
		}
		if (loc != null) {
			mappingVectorNextLine.addElement(new MappingObject.LeftParen(loc));
		} else {
			addLeftParen(ast.getOrigin());
		}
	}
	...
	/**
	 * Like addRightParen(ast.getOrigin()), except that it uses loc as the location
	 * if it is not null. It is called by GenLabeledStmt and ast.getOrigin() should
	 * never be null. However, if it is, then we don't add a Paren; this insures
	 * that the matching calls of addLeftParenV and addRightParenV both either do or
	 * don't add a Paren.
	 * 
	 * @param ast
	 */
	private void addRightParenV(AST ast, DistPCalLocation loc) {
		if (ast.getOrigin() == null) {
			return;
		}
		if (loc != null) {
			mappingVectorNextLine.addElement(new MappingObject.RightParen(loc));
		} else {
			addRightParen(ast.getOrigin());
		}
	}
	/* -------------------------------------------------------------------------- */
	/*
	 * The following methods and classes of objects are used in GenSpec(). See the
	 * comments preceding that method above.
	 */
	...
	public static class FormulaPair {
		...
		/**
		 * The representation of the conjunction of the formulas with prefix /\s, where
		 * the first /\ appears in column col (Java numbering), witout any ending "\n"
		 * 
		 * @return
		 */
	...
	/**
	 * Describes a process fairness formula, as described in the comments preceding
	 * the GetSpec() method above.
	 * 
	 * @author lamport
	 *
	 */
	public static class NodeFairness {
		 ...
		 * @param prcdVal
		 */
		public NodeFairness(String xfVal, Vector prefixVal, String bodyWF, String bodySF, Vector prcdVal) {
			xf = xfVal;
			prefix = prefixVal;
			bodyFormulas = null;
			if (bodyWF != null) {
		...
		/**
		 * The width of the fairness formula written as a "single-line" formula.
		 * Single-line means that it is not written as a conjunction list (with a
		 * leading /\). It will actually occupy multiple lines if prefix is a multi-line
		 * formula.
		 * 
		 * @return
		 */
		...
		/**
		 * Returns the prefix as a StringBuffer, assuming it starts in column col. That
		 * is, all but the first line is indented with col spaces, and all but the last
		...
		/**
		 * The process fairness condition written as a single-line formula, starting in
		 * column col.
		 * 
		 * @return
		 */
		...
		/**
		 * Returns true iff format(col) should return a single-line version of the
		 * formula.
		 * 
		 * @param col
		 * @return
		...
		private boolean fitsAsSingleLine(int col) {
			return (col + singleLineWidth() <= DistPcalTLAGen.wrapColumn)
					|| (bodyFormulas.sf == null && (prcdFormulas == null || prcdFormulas.size() == 0));
		}

		/**
		 * The process fairness condition written as a formula that begins in column col
		 * (Java numbering) and ends with "\n". It is formatted to try to extend no
		 * further than column PcalTLAGen.wrapColumn, but no individual formula is split
		 * across lines.
		 * 
		 * @param col
		 * @return
		...
		public StringBuffer format(int col) {
			...
			/*
			 * Return the single-line form if either it fits on the line or if it consists
			 * of only the wf formula (so it can't be put on multiple lines).
			 */
			...
			if (curCol + line.length() + 3 <= DistPcalTLAGen.wrapColumn) {
				val.append("/\\ " + line);
			} else {
