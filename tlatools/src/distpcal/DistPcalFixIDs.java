...
public class DistPcalFixIDs {
    private static DistPcalSymTab st = null;

    public static void Fix(AST ast, DistPcalSymTab stab) throws DistPcalFixIDException {
        st = stab;
        FixSym(ast, "");
        // Fix entry point for uniprocess algorithm
        if (st.iPC != null)
            st.iPC = st.UseThis(DistPcalSymTab.LABEL, st.iPC, "");
    }

    private static void FixSym (AST ast, String context) throws DistPcalFixIDException {
        ...
        *******************************************************************/
        ...
        else DistPcalDebug.ReportBug("Unexpected AST type" + ast.toString());
    }

    private static void FixUniprocess (AST.Uniprocess ast, String context) throws DistPcalFixIDException {
    ...
    private static void FixMultiNodes (AST.MultiNodes ast, String context) throws DistPcalFixIDException {
        for (int i = 0; i < ast.decls.size(); i++)
            FixVarDecl((AST.VarDecl) ast.decls.elementAt(i), "");
     ...
     * @param ast
     * @param context
     * @throws DistPcalFixIDException
     */
    private static void FixProcedure (AST.Procedure ast, String context) throws DistPcalFixIDException {
        for (int i = 0; i < ast.decls.size(); i++)
        ...
        DistPcalSymTab.ProcedureEntry p = 
            (DistPcalSymTab.ProcedureEntry)
            st.procs.elementAt(st.FindProc(ast.name));
        for (int i = 0; i < ast.plusLabels.size(); i++) {
        	String lbl = (String) ast.plusLabels.elementAt(i);
        	String newLbl = st.UseThis(DistPcalSymTab.LABEL, lbl, ast.name);
       ...
       for (int i = 0; i < ast.minusLabels.size(); i++) {
        ...
        p.iPC = st.UseThis(DistPcalSymTab.LABEL, p.iPC, ast.name);
        ast.name = st.UseThis(DistPcalSymTab.PROCEDURE, ast.name, "");
        p.name = ast.name;
    }
     ...
     * @throws DistPcalFixIDException
     */
    private static void FixNode(AST.Node ast, String context) throws DistPcalFixIDException {
        for (int i = 0; i < ast.decls.size(); i++)
       ...
       String temp = st.UseThis(DistPcalSymTab.LABEL, p.iPC.toString(), ast.name);
       ...
       for(int i = 0; i < stringArray.length; i++) {
        ...
        p.iPC = sb;
        ast.name = st.UseThis(DistPcalSymTab.NODE, ast.name, "");
        p.name = ast.name;
   }

    private static void FixVarDecl(AST.VarDecl ast, String context) throws DistPcalFixIDException {
        ast.var = st.UseThisVar(ast.var, context);
        FixExpr(ast.val, context);
    }

    private static void FixPVarDecl(AST.PVarDecl ast, String context) throws DistPcalFixIDException {
        ast.var = st.UseThisVar(ast.var, context);
        FixExpr(ast.val, context);
    }

    private static void FixLabeledStmt(AST.LabeledStmt ast, String context) throws DistPcalFixIDException {
        ast.label = st.UseThis(DistPcalSymTab.LABEL, ast.label, context);
        for (int i = 0; i < ast.stmts.size(); i++)
            FixSym((AST) ast.stmts.elementAt(i), context);
    }

    private static void FixWhile(AST.While ast, String context) throws DistPcalFixIDException {
    ...
    private static void FixAssign(AST.Assign ast, String context) throws DistPcalFixIDException {
        for (int i = 0; i < ast.ass.size(); i++)
            FixSingleAssign((AST.SingleAssign) ast.ass.elementAt(i), context);
    }

    private static void FixSingleAssign(AST.SingleAssign ast, String context) throws DistPcalFixIDException {
        FixLhs(ast.lhs, context);
        FixExpr(ast.rhs, context);
    }

    private static void FixLhs(AST.Lhs ast, String context) throws DistPcalFixIDException {
        ast.var = st.UseThisVar(ast.var, context);
        FixExpr(ast.sub, context);
    }

    private static void FixIf(AST.If ast, String context) throws DistPcalFixIDException {
    ...
    private static void FixWith(AST.With ast, String context) throws DistPcalFixIDException {
        FixExpr(ast.exp, context);
        for (int i = 0; i < ast.Do.size(); i++)
            FixSym((AST) ast.Do.elementAt(i), context);
    }

    private static void FixWhen(AST.When ast, String context) throws DistPcalFixIDException {
        FixExpr(ast.exp, context);
   }

    private static void FixPrintS(AST.PrintS ast, String context) throws DistPcalFixIDException {
        FixExpr(ast.exp, context);
    }

    private static void FixAssert(AST.Assert ast, String context) throws DistPcalFixIDException {
        FixExpr(ast.exp, context);
   }
    ...
    private static void FixLabelIf(AST.LabelIf ast, String context) throws DistPcalFixIDException {
    ...
    private static void FixCall(AST.Call ast, String context) throws DistPcalFixIDException {
        ast.returnTo = st.UseThis(DistPcalSymTab.PROCEDURE, ast.returnTo, context);
        ast.to = st.UseThis(DistPcalSymTab.PROCEDURE, ast.to, context);
        for (int i = 0; i < ast.args.size(); i++)
            FixExpr((TLAExpr) ast.args.elementAt(i), context);
    }

    private static void FixReturn(AST.Return ast, String context) {
        ast.from = st.UseThis(DistPcalSymTab.PROCEDURE, ast.from, context);
    }

    private static void FixCallReturn(AST.CallReturn ast, String context) throws DistPcalFixIDException {
        ast.from = st.UseThis(DistPcalSymTab.PROCEDURE, ast.from, context);
        ast.to = st.UseThis(DistPcalSymTab.PROCEDURE, ast.to, context);
        for (int i = 0; i < ast.args.size(); i++)
            FixExpr((TLAExpr) ast.args.elementAt(i), context);
    ...
    private static void FixCallGoto(AST.CallGoto ast, String context) throws DistPcalFixIDException {
        ast.after = st.UseThis(DistPcalSymTab.PROCEDURE, ast.after, context);
        ast.to = st.UseThis(DistPcalSymTab.PROCEDURE, ast.to, context);
        for (int i = 0; i < ast.args.size(); i++)
            FixExpr((TLAExpr) ast.args.elementAt(i), context);
    }

    private static void FixGoto(AST.Goto ast, String context) throws DistPcalFixIDException {
        /*
         * Report an error if the goto destination is not a label.  This check
         * added by LL on 29 Dec 2011.
         */
        if (st.FindSym(DistPcalSymTab.LABEL, ast.to, context) == st.symtab.size()
              && ! ast.to.equals("Done")) {
            throw new DistPcalFixIDException("goto to non-existent label `" + ast.to + 
                    "' at line " + ast.line + ", column " + ast.col);
        }
        ast.to = st.UseThis(DistPcalSymTab.LABEL, ast.to, context);
    }
    ...
    * Handling of Either and LabelEither added by LL on 24 Jan 2006.       
     * @throws DistPcalFixIDException *
    ***********************************************************************/
    private static void FixEither(AST.Either ast, String context) throws DistPcalFixIDException {
    ...
    private static void FixLabelEither(AST.LabelEither ast, String context) throws DistPcalFixIDException {
    ...
    private static void FixExpr(TLAExpr expr, String context) throws DistPcalFixIDException {
        /**
        ...
        */
        ...
        if (exprVec.size() > 0)
            ...
            {
                throw new DistPcalFixIDException(e.getMessage());
            }
    }
...
private static void FixMultiprocess (AST.Multiprocess ast, String context) throws DistPcalFixIDException {
    for (int i = 0; i < ast.decls.size(); i++)
        FixVarDecl((AST.VarDecl) ast.decls.elementAt(i), "");
 ...
 */
private static void FixProcess(AST.Process ast, String context) throws DistPcalFixIDException {
	for (int i = 0; i < ast.decls.size(); i++)
		FixVarDecl((AST.VarDecl) ast.decls.elementAt(i), ast.name);
	for (int i = 0; i < ast.body.size(); i++)
		FixLabeledStmt((AST.LabeledStmt) ast.body.elementAt(i), ast.name);
