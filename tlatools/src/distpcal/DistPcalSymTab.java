...
public class DistPcalSymTab {
	...
	// For toString method.
	public static String vtypeName[] = { "Global variable", "Label", "Procedure", "Process", "Process variable",
			"Procedure variable", "Parameter", "Node" };
	...
	public class SymTabEntry {
		...
		public String toString() {
			return "[id: " + this.id + ", usethis: " + this.useThis + ", line: " + this.line + ", col:" + this.col
	...
	/**
	 * As should be perfectly obvious from the name, this method constructs the
	 * symbol table for the AST ast, which I presume contains all things whose name
	 * must be looked up, which includes labels, variables, and probably process and
	 * procedure names.
	 * 
	 * @param ast
	 * @throws DistPcalSymTabException
	 */
	public DistPcalSymTab(AST ast) throws DistPcalSymTabException {
...
//        InsertSym(LABEL, "Done", "", "", 0, 0);
		...
		if (errorReport.length() > 0)
			throw new DistPcalSymTabException(errorReport);
	}
	...
	/***************************************************
	 * TRUE if inserted; false if was already in table * Can not insert a variable
	 * of name x if there is * a global with name x or another variable in the *
	...
	public boolean InsertSym(int type, String id, String context, String cType, int line, int col) {
		...
		if (type == PROCEDUREVAR || type == PROCESSVAR || type == PARAMETER) {
			i = FindSym(GLOBAL, id, "");
			if (i < symtab.size())
				return false; /* GLOBAL with same id exists */
			i = FindSym(id, context);
			if (i < symtab.size())
				return false; /* id in same context exists */
		} else {
	...
	/***************************************************
	 * Insert procedure table entry. * TRUE if inserted; false if was already in
	 * table *
	 ***************************************************/
	...
	/*********************************************************
	 * Various ways to look up something in the symbol table.* Returns the index in
	 * the table. If the index equals * symtab.size() (which makes no sense), then
	 * it isn't * in the symbol table. *
	 *********************************************************/
	...
	/*********************************************************
	 * Returns index of entry in procedure table. If it isn't* in the table, then
	 * returns procs.size(). *
	 *********************************************************/
	...
	public String UseThis(int type, String id, String context) {
		...
		if (i == symtab.size())
			...
			return ((SymTabEntry) symtab.elementAt(i)).useThis;
	}

	
//	/* Return the disambiguated name for a type X id X context */
//	public StringBuffer UseThis(int type, StringBuffer ids, String context) {
//		
//		int i = 0;
//		
//		for(String id : ids) {
//			FindSym(type, id, context);
//		}
//		if (i == symtab.size())
//			return id;
//		else
...
//	}
	...
	/*********************************************************
	 * Given a variable referenced in a context. First get * the entry in the
	 * context. If no such variable, then * see if a global one exists. If not, then
	 * use it * undisambiguated. * NOTE: Stop using FindSym; it's a bad hack. *
	 *********************************************************/
	public String UseThisVar(String id, String context) {
		...
		if (se.type == GLOBAL || se.type == PROCESSVAR || se.type == PROCEDUREVAR || se.type == PARAMETER)
			return se.useThis;
		i = FindSym(id, "");
		if (se.type == GLOBAL)
			return se.useThis;
	...
	public String UseThis(String id, String context) {
		...
		while (i < symtab.size()) {
			SymTabEntry se = (SymTabEntry) symtab.elementAt(i);
			if (se.id.equals(id) && se.context.equals(context)
					&& (se.type == GLOBAL || se.type == PROCESSVAR || se.type == PROCEDUREVAR || se.type == PARAMETER))
				break;
			i = i + 1;
		}
		if (i == symtab.size())
	 ...
	 * string, defined in the array typePrefix. Then, the names are * considered in
	 * increasing type order. The first type in this order * are left unchanged.
	 * Then, each other order has a suffix appended to it * until it is unique from
	 * all the current symbol names (both renamed and * not yet renamed). The suffix
	 * is a prefix of "_context" where "context" * is the context in which the name
	 * is defined (which is the empty string * for the global context). If it is
	 * still not unique after all of the * context is appended, then the decimal
	 * representation of the name type * is added repeatedly until it is
	 * unambiguous. *
	 **************************************************************************/
			...
			for (int i = 0; i < symtab.size(); i++) {
				...
				if (se.type == vtype) {
					...
					if (!se.id.equals(se.useThis))
						disambiguateReport.addElement("\\* " + vtypeName[se.type] + " " + se.id
								+ ((se.context.length() == 0) ? "" : (" of " + se.cType + " " + se.context))
								+ " at line " + se.line + " col " + se.col + " changed to " + se.useThis);
				}
			}
	}
	...
	public String toString() {
		...
		while (i < symtab.size()) {
			...
			if (i > 0)
				result = result + ", ";
			result = result + vtypeName[se.type] + " " + se.context + ':' + se.id + " line " + se.line + " col "
					+ se.col + " (" + se.useThis + ")";
			i = i + 1;
		}
		return result + "]";
	...
	private void ExtractSym(AST ast, String context) {
		if (ast.getClass().equals(AST.MultiNodesObj.getClass()))
	        ...
	        DistPcalDebug.ReportBug("Unexpected AST type.");
		}
	}
	...
	private void ExtractStmt(AST ast, String context, String cType) {
		if (ast.getClass().equals(AST.WhileObj.getClass()))
			ExtractWhile((AST.While) ast, context, cType);
				...
				|| ast.getClass().equals(AST.SingleAssignObj.getClass()))
...
// detected.
	...
	private void ExtractProcedure(AST.Procedure ast, String context) {
		...
		if (!InsertProc(ast))
			errorReport = errorReport + "\nProcedure " + ast.name + " redefined at line " + ast.line + ", column "
					+ ast.col;
		boolean b = InsertSym(PROCEDURE, ast.name, context, "procedure", ast.line, ast.col);
		for (int i = 0; i < ast.decls.size(); i++)
		...
		for (int i = 0; i < ast.body.size(); i++)
			ExtractLabeledStmt((AST.LabeledStmt) ast.body.elementAt(i), ast.name, "procedure");
	}

	private void ExtractProcess(AST.Process ast, String context) {
		boolean b;
		if (!InsertProcess(ast))
			errorReport = errorReport + "\nProcess " + ast.name + " redefined at line " + ast.line + ", column "
					+ ast.col;
		b = InsertSym(PROCESS, ast.name, context, "process", ast.line, ast.col);
		for (int i = 0; i < ast.decls.size(); i++)
			ExtractVarDecl((AST.VarDecl) ast.decls.elementAt(i), ast.name);
		for (int i = 0; i < ast.body.size(); i++)
			ExtractLabeledStmt((AST.LabeledStmt) ast.body.elementAt(i), ast.name, "process");
	}
	...
	private void ExtractParamDecl(AST.PVarDecl ast, String context) {
		if (!InsertSym(PARAMETER, ast.var, context, "procedure", ast.line, ast.col))
			errorReport = errorReport + "\nParameter " + ast.var + " redefined at line " + ast.line + ", column "
					+ ast.col;
	}
	...
	private void ExtractLabeledStmt(AST.LabeledStmt ast, String context, String cType) {
		if (!InsertSym(LABEL, ast.label, context, cType, ast.line, ast.col))
			errorReport = errorReport + "\nLabel " + ast.label + " redefined at line " + ast.line + ", column "
					+ ast.col;
	...
	private void ExtractWhile(AST.While ast, String context, String cType) {
		...
		for (int i = 0; i < ast.labDo.size(); i++)
			ExtractLabeledStmt((AST.LabeledStmt) ast.labDo.elementAt(i), context, cType);
	}
	...
	private void ExtractLabelIf(AST.LabelIf ast, String context, String cType) {
		...
		for (int i = 0; i < ast.labThen.size(); i++)
			ExtractLabeledStmt((AST.LabeledStmt) ast.labThen.elementAt(i), context, cType);
		...
		for (int i = 0; i < ast.labElse.size(); i++)
			ExtractLabeledStmt((AST.LabeledStmt) ast.labElse.elementAt(i), context, cType);
	}
	...
	private void ExtractLabelEither(AST.LabelEither ast, String context, String cType) {
		for (int i = 0; i < ast.clauses.size(); i++) {
			...
			for (int j = 0; j < orClause.unlabOr.size(); j++)
				ExtractStmt((AST) orClause.unlabOr.elementAt(j), context, cType);
			for (int j = 0; j < orClause.labOr.size(); j++)
				ExtractLabeledStmt((AST.LabeledStmt) orClause.labOr.elementAt(j), context, cType);
		}
		;
	}

	/************************************************************************
	 * Reports an error if "defaultInitValue" appears in the symbol table. * Added
	 * by LL on 31 Aug 2007. *
	 ************************************************************************/
	public void CheckForDefaultInitValue() throws DistPcalSymTabException {
		String errors = "";
		for (int i = 0; i < symtab.size(); i++) {
			SymTabEntry se = (SymTabEntry) symtab.elementAt(i);
		...
		;
		if (!errors.equals("")) {
			throw new DistPcalSymTabException(errors);
		}
		;
		return;
	}
