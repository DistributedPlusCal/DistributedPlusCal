...
/************* made public for DEBUGGING  ****************/
    public static DistPcalCharReader reader ;
      /*********************************************************************
    ...
    ***********************************************************************/
                ...
                ***********************************************************/
                if (   (type != Token.STRING)
                 ...
                 { /********************************************************
                   ...
                   ********************************************************/
                   if (type == Token.BUILTIN)
                     { Symbol sym = DistPcalBuiltInSymbols.GetBuiltInSymbol(token);
    ...
    public static TLAExpr TokenizeExpr(DistPcalCharReader charReader) throws TokenizerException
      { TLAExpr exp = InnerTokenize(charReader, true) ;
        return exp ; }

    public static String GetAlgorithmToken(DistPcalCharReader charReader) throws TokenizerException { 
    	TLAExpr exp = InnerTokenize(charReader, false) ;
    	return Delimiter ;
    }

    public static TLAExpr InnerTokenize(DistPcalCharReader charReader, 
        ...
        *******************************************************************/
        ...
        switch (mode)
          { case MODULE : state = PROLOG ; break ;
            ...
            default     : DistPcalDebug.ReportBug(
        	...
        	switch (state) 
              ...
              *************************************************************/
              { case START :
                  ...
                  else if (DistPcalBuiltInSymbols.IsBuiltInPrefix("" + nextChar))
                    { addNextChar();
                      state = BUILT_IN ;
                    }
                  else if (nextChar == '\t')
                ...
                case ID :
                  ...
                  else if (DistPcalBuiltInSymbols.IsBuiltInSymbol(token))
                ...
                case BUILT_IN :
                  ...
                  if (DistPcalBuiltInSymbols.IsBuiltInPrefix(token + nextChar))
                  { 
                    ...
                    { 
                     if (!DistPcalBuiltInSymbols.IsBuiltInSymbol(token))
                      {

                        reader.backspace();
                        while (! DistPcalBuiltInSymbols.IsBuiltInSymbol(token))
                        { reader.backspace();
                          if (token.length() == 0)
                            { TokenizingError("Illegal lexeme");
                            };
                ...
                case STRING :
                  ...
                  else if (DistPcalBuiltInSymbols.IsStringChar(nextChar))
                    { addNextChar();
                      state = STRING ;
                    }
                  else
                ...
                case PROLOG_DASH :
                	DistPcalDebug.Assert(token.length() <= 3) ;
                  if (nextChar ==   '-')
                    { addNextChar();
                      if (token.length() == 4)
                        { state = PROLOG_DASHES;
