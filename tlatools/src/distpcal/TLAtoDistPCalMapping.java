 ...
 * <p>
 * The TLA+ to PlusCal mapping is computed and put in the {@link TLAtoDistPCalMapping#mapping}
 ...
 *
 */
public class TLAtoDistPCalMapping implements Serializable {
	/**
	 * If {@link TLAtoDistPCalMapping#mapping} is changed or additional
  ...
  public void  makeMapping(Vector<Vector<MappingObject>> mapVec) {
	...
	/**
	 * Calls {@link TLAtoDistPCalMapping#ApplyMapping(TLAtoDistPCalMapping, Region)} to
	 * find the distPcal {@link Region}, using the {@link Region} tlaRegion obtained
	 * by adjusting the line numbers of the selected region by
	 * currentAlgorithmLine.
	 * 
	 * @param tlaRegion
	 *            The TLA {@link Region} for which the distPcal {@link Region} is to
	 *            be returned
	 * @param currentAlgorithmLine
	 *            The current line of the "--algorithm" or "--fair" keyword in
	 *            the module (in Java coordinates)
	 * @return The {@link Region} of the distPcal counterpart
	 * @see TLAtoDistPCalMapping#ApplyMapping(TLAtoDistPCalMapping, Region)
	 */
	public Region mapTLAtoDistPCalRegion(final Region tlaRegion, int currentAlgorithmLine) {
		...
		// b) Actually map TLA to distPcal based on the translation
		final Region distPcalRegion = ApplyMapping(this, tlaRegion);
		...
		if (distPcalRegion != null) {
			distPcalRegion.getBegin().adjustLineBy(delta);
			distPcalRegion.getEnd().adjustLineBy(delta);
		}
		
		return distPcalRegion;
	}
	...
	public boolean equals(Object obj) {
		...
		if (getClass() != obj.getClass())
			return false;
		TLAtoDistPCalMapping other = (TLAtoDistPCalMapping) obj;
     ...
     */
	public static int GetLineOfDistPCalAlgorithm(String moduleAsString) {
		final int algorithmStringLocation = moduleAsString.indexOf(DistPcalParams.BeginAlg);
		final int fairStringLocation = moduleAsString.indexOf(DistPcalParams.BeginFairAlg);
		...
		int i = fairStringLocation + DistPcalParams.BeginFairAlg.length();
		while (Character.isWhitespace(moduleAsString.charAt(i))) {
			i++;
		}
		if ((i != fairStringLocation + DistPcalParams.BeginFairAlg.length())
				&& (moduleAsString.startsWith(DistPcalParams.BeginFairAlg2, i))
				&& !Character.isLetterOrDigit(moduleAsString.charAt(i + DistPcalParams.BeginFairAlg2.length()))) {
...
/**
   * Returns the distPcal code location to which `mapping' maps the tpregion Region in the
   * TLA+ translation, where line numbers in `selection' are relative to tlaStartLine.
   * It returns null if the mapping does not map the selection to any distPcal code. 
   ...
   * @return
   */
  public static Region ApplyMapping(TLAtoDistPCalMapping mapping, Region tpregion) {
      /*
...
//MappingObject.printMapping(map);
      ...
      DistPCalLocation ltok = tokPairRegion.getBegin();
      DistPCalLocation rtok = tokPairRegion.getEnd();
      ...
      DistPCalLocation i = NextLocOf(ltok, tpMap);
      while (LTEq(i, rtok)) {
   ...
   * @return
   */
  private static int ModifiedDepth(int var, DistPCalLocation pos, boolean movingForward, MappingObject[][] tpMap) {
      int amt = 0;
  ...
  private static Region RegionToTokPair(MappingObject[][] spec, Region reg) {
       ...
       */
      DistPCalLocation regBegin = reg.getBegin();
      DistPCalLocation regEnd = reg.getEnd();
      /*
      ...
      if (regBegin.getLine() < 0) {
          regBegin = new DistPCalLocation(0, 0) ;
      }
      
      if (regEnd.getLine() >= spec.length) {
          regEnd = new DistPCalLocation(spec.length-1,999);
      }
       ...
       * 
       */
      DistPCalLocation tokAtOrRightOfBeginning = null ;
      ...
      DistPCalLocation loc = new DistPCalLocation(locLine, 0);
      DistPCalLocation prevloc = null ;
      ...
      while (notDone && loc != null) {
          if (prevIsBeginToLeft) {
              ...
              if (LT(regBegin, new DistPCalLocation(loc.getLine(), mobj.getColumn()))) {
              ...
              if (obj.getType() == MappingObject.END_TLATOKEN) {
                 ...
                 if (LT(regBegin, new DistPCalLocation(loc.getLine(), eobj.getColumn()))) {
                  ...
                  if (LTEq(regBegin, new DistPCalLocation(loc.getLine(), bobj.getColumn()))) {
                  ...
                  if (LT(regBegin, new DistPCalLocation(loc.getLine(), sobj.getEndColumn()))) {
                      tokAtOrRightOfBeginning = loc;
                      notDone = false;
                  }
              }
      ...
      DistPCalLocation tokAtOrLeftOfEnd = null ;
    ...
    loc = new DistPCalLocation(locLine, spec[locLine].length-1);
    prevloc = null ;
    ...
    while (notDone && loc != null) {
        if (prevIsEndToRight) {
            MappingObject.BeginTLAToken mobj 
              ...
              // This will throw an exception if a BeginTLAToken is not
              // immediately followed by an EndTLAToken
            if (LT(new DistPCalLocation(loc.getLine(), mobj.getColumn()), regEnd)) {  // should be LT
            ...
            if (obj.getType() == MappingObject.BEGIN_TLATOKEN) {
               ...
               if (LT(new DistPCalLocation(loc.getLine(), eobj.getColumn()), regEnd)) {
                ...
                if (LTEq(new DistPCalLocation(loc.getLine(), bobj.getColumn()), regEnd)) {
                ...
                MappingObject.SourceToken sobj = (MappingObject.SourceToken) obj;
                if (LT(new DistPCalLocation(loc.getLine(), sobj.getBeginColumn()), regEnd)) {
                    tokAtOrLeftOfEnd = loc;
                    notDone = false;
                }
            }
    ...
    if (tokAtOrRightOfBeginning != null) {
        if (tokAtOrLeftOfEnd == null) {
             ...
             */
             ...
             if (obj.getType() == MappingObject.END_TLATOKEN) {
                 distToLeft = Dist(new DistPCalLocation (tokAtOrLeftOfEnd.getLine(), 
             ...
             if (obj.getType() == MappingObject.BEGIN_TLATOKEN) {
                 distToRight = Dist(new DistPCalLocation (tokAtOrRightOfBeginning.getLine(), 
              ...
              */
             DistPCalLocation leftBegin = null;
             DistPCalLocation rightEnd = null;
   ...
   * @return
   */
  private static MappingObject ObjectAt(DistPCalLocation loc, MappingObject[][] map) {
   ...
   * @return
   */
  private static DistPCalLocation PrevLocOf(DistPCalLocation loc, MappingObject[][] map) {
      if (loc.getColumn() > 0) {
          return new DistPCalLocation(loc.getLine(), loc.getColumn()-1) ;
      }
      for (int i = loc.getLine() - 1; i >= 0; i--) {
          if (map[i].length > 0) {
              return new DistPCalLocation(i, map[i].length - 1);
          }
      }
      return null;
  }
   ...
   * @return
   */
  private static DistPCalLocation NextLocOf(DistPCalLocation loc, MappingObject[][] map) {
      if (loc.getColumn() + 1 < map[loc.getLine()].length) {
          return new DistPCalLocation(loc.getLine(), loc.getColumn()+1) ;
      }
      for (int i = loc.getLine() + 1; i < map.length; i++) {
          if (map[i].length > 0) {
              return new DistPCalLocation(i, 0);
          }
      }
      return null;
  }
   ...
   * @return
   */
  private static boolean LTEq(DistPCalLocation locA, DistPCalLocation locB) {
   ...
   * @return
   */
  private static boolean LT(DistPCalLocation locA, DistPCalLocation locB) {
      if (locA.getLine() == locB.getLine()) {
          return locA.getColumn() < locB.getColumn();
      }
      return locA.getLine() < locB.getLine();
   ...
   * @return
   */
  private static int Dist(DistPCalLocation locA, DistPCalLocation locB) {
   ...
   * corresponded to line 0 of {@link TLAtoDistPCalMapping#mapping}.  At that
  ...
  public static Vector<Vector<MappingObject>> RemoveRedundantParens(Vector<Vector<MappingObject>> mappingVec) {
      ...
      Vector<DistPCalLocation> unmatchedLeft = new Vector<DistPCalLocation>();  
         // Vector of PCalLocations in out of unmatched LeftParen objects
      DistPCalLocation lastMatchedLeft = null;
        // position in out of last LeftParen object that was matched by
        // a RightParen object
      DistPCalLocation lastAddedRight = null;
      int i = 0 ;
          ...
          while (j < inLine.size()) {
              ...
              if (inObj.getType() == MappingObject.LEFT_PAREN) {
                 ...
                 */
                  ...
                  lastAddedRight = new DistPCalLocation(i, j);
              }
   ...
   * @return
   */
  private static boolean IsNextIn(DistPCalLocation locA, DistPCalLocation locB, Vector<Vector<MappingObject>> vec) {
      return     (locA != null)
              && (locB != null)
...
(***************************************************************************)
(* TLA to distPcal translation objects.                                        *)
(***************************************************************************)
...
(* determining the distPcal region that maps to a part of the TLA+ spec.       *)
(*                                                                         *)
...
(* regions in the same part of the distPcal text.  (Code in a macro and code   *)
(* in a procedure is an example of two regions in completely different     *)
(* parts of the distPcal code, and hence are not successive regions.) Two      *)
...
(* that represent distPcal code in which the region between the parenthesized  *)
...
-----------------------------------------------------------------------------
(***************************************************************************)
(* The Region in the distPcal code specified by a Region in the TLA+ spec.     *)
(***************************************************************************)
...
the distPcal code that, according to the mapping specification tpMap,
