...
jar_parse = "dist/tla2tools.jar"

# jar location for checking test
jar_check =  "dist/tla2tools.jar"

# jar location for sanity test
jar_sanity =  "dist/tla2tools.jar"

# path where all the tests are located
main_path = "testdp-source"
...
result_path = "testdp-result"
# path where all compiled specifications are saved
compile_path = "testdp-compile"
...
def safe_makedirs(path):
    """Create a directory path if needed, ignoring races/duplicates."""
    os.makedirs(path, exist_ok=True)


def extract_translation_body(tla_file_path):
    """Return content between BEGIN/END TRANSLATION markers from a TLA file."""
    chunks = []
    in_translation = False
    with open(tla_file_path, "r") as tla_file:
        for line in tla_file:
            if line.startswith("\\* BEGIN TRANSLATION"):
                in_translation = True
                continue
            if in_translation and line.startswith("\\* END TRANSLATION"):
                in_translation = False
                continue
            if in_translation:
                chunks.append(line)
...
def run_compare(tla_file_path_1_in, tla_file_path_2_in):
    # Compare only translator output blocks to avoid irrelevant header/footer diffs.
    str_1 = extract_translation_body(tla_file_path_1_in)
    str_2 = extract_translation_body(tla_file_path_2_in)
...
def create_cfg(path, file_name, constants):
    ...
    # write tla file
    with open(os.path.join(path, test_file_name + ".tla"), "w") as test_file_tla:
    ...
    # write cfg file
    with open(os.path.join(path, test_file_name + ".cfg"), "w") as test_file_cfg:
        # write each constant
        for k, v in constants.items():
...
def remove_cfg(path, file_name):
    ...
    try:
        ...
        if verbose_level >= 2:
            print("\n    Warning: Missing original TLA file in compile directory")
    ...
    try:
        ...
        if verbose_level >= 2:
            print("\n    Warning: Missing generated", GEN_FILE_PREFIX, "files")
...
def process_result(test_name,completed_process,phase,expect_error):
    ok = phase+'_ok';
    error = phase+'_error';
    ...
    elif completed_process == "error":
        tests[test_name][ok] = False
        tests[test_name][error] = "Error"
    else:
        code = completed_process.returncode
        if code == 0: # process succeeded 
            if expect_error: # error was expected (instead of correct behaviour)
                tests[test_name][ok] = False
            ...
            if expect_error: # and this was expected
                tests[test_name][ok] = True
    ...
    if verbose_level >= 2:
        if tests[test_name][ok]:
            print(' --> ',phase, colored(' OK', "green"))
        else:
            print(' --> ',phase, colored(' ERROR', "red"))
...
def check_args():
    ...
    while i < len(sys.argv):
        if sys.argv[i] in ('-h', "--help"):
            print(sys.argv[0], "[-h] [-jp <jar_path>] [-jc <jar_path>] [-v <0..4>] [-tp <test_path>] [-dc] [-o]")
            exit()
            ...
            print(sys.argv[i], " not allowed")
            print(sys.argv[0], "[-h] [-jp <jar_path>] [-jc <jar_path>] [-v <0..4>] [-tp <test_path>] [-dco] [-dck] [-dg] [-o]")
            exit()
        i += 1
...
# Keep only .tla files that live under a subdirectory of test_path named test-*.
search_root = os.path.join(test_path, specific_path) if specific_path else test_path
candidate_files = glob.glob(os.path.join(search_root, "**", "*.tla"), recursive=True)
tmp_files = sorted(
    f for f in candidate_files
    if any(part.startswith("test-") for part in os.path.relpath(f, test_path).split(os.sep)[:-1])
)
...
for f in tmp_files:
    ...
    if not base_name.startswith(GEN_FILE_PREFIX) and os.path.isfile(f):
        ...
        shutil.copy(f, path)
        reldir = os.path.relpath(dir_name, test_path)
        tests[base_name] = {"path": path, "dir": reldir}
# print tests found
if verbose_level >= 5:
    print("--- Tests found ---")
...
#  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Init phase
# - extract test options (or set to default)
if verbose_level >= 1:
    print('--- Init ---')
for t in tests:
    test = tests[t]
    ...
    # Read optional JSON test options between two module separator lines (==== ... ====).
    try:
        json_str = ""
        ...
        # else:
            # default options
            # test["json"] = {"expect-error-parse": False, "expect-error-check": False}
...
#  - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Compile  phase
# - compile tests that had been copied to the compile directory
if verbose_level >= 1:
    print('--- Translate  ---')
for t in tests:
    test = tests[t]
    ...
    expect_error = False
    if "expect-error-parse" in test["json"]:
        expect_error = test["json"]["expect-error-parse"]
    # process the result of the command
    process_result(t,completed_process,'compile',expect_error)
...
if do_check:
    ...
    for t in tests:
        ...
        if test["compile_ok"] and test['compile_error']=="ok":
            if verbose_level >= 2:
                print(" ", t, end="", flush=True)
            ...
            # check if an error is expected 
            expect_error = False
            if "expect-error-check" in test["json"]:
                expect_error = test["json"]["expect-error-check"]
            # process the result of the command
            process_result(t,completed_process,'check',expect_error)
        else:
            test['check_ok'] = False
...
# Compare phase
    # "compare_path": 
    #     not present in json --> use default compare_path/dir_du_test
    #     "compile"           --> use compile_path
    #     "<path>"            --> use <path>
...
if do_compare:
    ...
    for t in tests: 
        ...
        if "compare_to" in test["json"]:
            ...
            # if we want to compare with the result of compiling the other version C vs Pascal 
            #   or use a different path than the default one
            compare_path_changed = False
            if "compare_path" in test["json"]: 
                compare_path_changed = True
                if test["json"]["compare_path"] == "compile":
                    current_compare_path = os.path.join(os.getcwd(), compile_path)
                else: 
                    current_compare_path = os.path.join(os.getcwd(), test["json"]["compare_path"])
            # if compare name not specified, use the name of the test file
            if compare_name == "":
                compare_name = t
            # if default path used, add the directory of the test
            if not compare_path_changed:
                compare_name = os.path.join(test["dir"],compare_name)
            compare_file = os.path.join(current_compare_path,compare_name)
            nb_test['compare'] += 1
            if verbose_level >= 2:
                print(" to ", compare_file, end="", flush=True)
        ...
        if verbose_level >= 2:
            if "compare_to" in test["json"]:
                if test['compare_ok']:
                    print(' --> ',"compare", colored(' OK', "green"))
                else:
                    print(' --> ',"compare", colored(' ERROR', "red"))
...
if verbose_level >= 2:
    for t in tests:
        ...
        comp = tests[t]["compile_ok"]
        if comp:
            result_message = "OK"
            if tests[t]['compile_error'] == "errorWasExpected":
            ...
            output_message(out_file,"  Compilation: " + result_message)
            if verbose_level >= 3:
                output_message(out_file,"    Error: " + tests[t]["compile_error"])
                if verbose_level >= 4:
                    output_message(out_file,"    " + tests[t]["compile_detail"])
...
if verbose_level >= 1:
    print("Parse test: ", nb_test['compile_ok'], "/", nb_test['compile'])
...
if not nb_test['compile_ok'] == nb_test['compile']:
    exit(1)
