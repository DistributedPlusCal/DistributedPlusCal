...
*** NoProcessNoLabelWhile
- dummy algorithm with NO processes and a WHILE loop
- should produce the same code with distpcal flag or not - OK
- code produced for C and P is different (with or without distpcal flag)
	

** Test One Process
*** OneProcessEmptyThreadP (C & P)
- dummy algorithm with ONE processes with EMPTY body
- compilation for C syntax different from the one with P syntax (and
  with bad ProcSet)
- does not translate without distpcal flag
...
*** OneProcessOneThreadPlusLabels
- use + and - on labels 
- should produce the same code (except for pc) with distpcal flag or not

*** OneProcessOneThreadSet (C & P)
- as OneProcessOneThread but use string identifiers for processes
- generates two labels (because of the -labels option; two ops on same variable)
- should produce the same code (except for pc) with distpcal flag or not
...
*** OneProcessMultiThreadMacroC.tla (C & P)
...
- three processes (1 instance)  with 1 or 2 threads
- each process has labels
...
- and call one procedure with labels - and +

*** NProcesses2ThreadsFairness2ProcSetC
...
- process is WF
- and calls two procedures with labels are - and +
*** NProcesses2ThreadsSfC
- 3 processes 2 threads each
- 1 process classic, 1 fair, 1 fair+

*** NProcesses2ThreadsNoStutteringC.tla
- 3 processes 2 threads each
- 1 process classic, 1 fair, 1 fair+

*** TwoProcessOneThreadLabelC.tla
- should be almost the same as without distpcal 
- some labels are renamed with distpcal but not without
...
- should produce the same code (except for pc,stack) with distpcal flag or not 
*** Procedures1p1tArray
- one process, id fixed
- uses arrays
- should produce the same code (except for pc,stack) with distpcal flag or not 
...
*** Procedures2p2t2p
- two processes two threads two procedures
	
*** Procedures2p2tCall
- two processes two threads
- one of the procedures call the other one
- should produce the same code (except for pc,stack) with distpcal flag or not

*** Procedures2p2tFail
- two processes one thread
- should produce the same code (except for pc,stack) with distpcal flag or not
- the procedure uses a variable local to process(es) pid and thus, can't be called from another process
*** Procedures2p2tFix
- two processes one thread
- should produce the same code (except for pc,stack) with distpcal flag or not
- the procedure uses a variable local to process(es) pid but the procedures are respectivelly called from the process defining the corresponding variables 
