...
\tlaplus is a specification formalism not a programming language, it uses expressions that might appear unfamiliar to most programmers, Thus PlusCal was proposed to accompany TLA+, for the reason that PlusCal used conventional program constructs.
...
The $pc$ variable is introduced by the translator to indicate the navigation between the actions for a process.
...
The variable $pc$ is defined and used as a program control variable, it's a function whose domain is $ProcSet$ such that each element is mapped to the predicate that represents the entry point of execution for the process.
...
\item Transform PlusCal labels into \tlaplus actions where an action is the TLA+ representation of the atomic operation beginning at that label. 
