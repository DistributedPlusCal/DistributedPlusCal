...
\ednote{sm}{The order of presentation is a little confusing. I'd first show how
  unordered channels are declared and used in DPlusCal, and only then show how
  they are translated to \tlaplus.}
...
\ednote{sm}{You haven't shown the corresponding PlusCal declaration.}
...
\end{minipage}\hfill
%
\edmargin{sm}{not a condition that is checked}
where $e \in IdSet$.
...
\edmargin{sm}{Remove ``chan' ='' in the translation.}
...
\tlaplus sequences contain operators that enable
%
\edmargin{sm}{Really? I'd simply say that sequences are ordered collections, analogous to lists.}
iterating over all their elements
%
and keeping track of the order between the elements. For instance, adding to a sequence corresponds to appending at the end of the sequence with the $Append$ operator, and removing the first element of a sequence can be accomplished with the $Tail$ operation. FIFO channels are represented as \tlaplus sequences. We take advantage of these operators to maintain the order in the FIFO channels.
...
\lstinline|fifo <identifier>[<dimensions>]| $\triangleq$
\end{minipage}\hfill
...
\ednote{sm}{Again, this shows the initialization of the channel in \tlaplus, not
  the PlusCal channel declaration as announced above.}
...
\begin{minipage}{.7\textwidth}
\begin{lstlisting}[frame = none, numbers = none]
chan' = [chan EXCEPT ![e] = Append(@, msg)]\end{lstlisting}

\end{minipage}\hfill
\edmargin{sm}{again, not checked (and you could have used @ also above)}
where $e \in IdSet$.
...
\edmargin{sm}{again, remove initial ``chan'=''}
\begin{minipage}{.3\textwidth}
...
\\\\
\edmargin{sm}{explain what limitations you mean}
